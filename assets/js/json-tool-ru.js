document.addEventListener('DOMContentLoaded', () => {
    ...
    const processJson = (isInitialLoad = false) => {
        ...
        try {
            ...
            updateStatus('Действительный JSON', false);
            renderOutputs();
            localStorage.setItem(STORAGE_KEY, jsonString);
        } catch (e) {
            currentJson = null;
            updateStatus(`Недействительный JSON: ${e.message}`, true);
            for (const key in outputs) {
                if (outputs[key]) {
                    if (key === 'tree') outputs[key].innerHTML = '';
                    else outputs[key].textContent = '';
    ...
    const renderOutputs = () => {
        ...
        try {
            ...
            outputs.base64.textContent = 'Ошибка при кодировании в Base64.';
        }
    ...
    function jsonToXml(json) {
        ...
        try {
            ...
            return `Ошибка при преобразовании в XML: ${e.message}`;
        }
    }
    ...
    function jsonToYaml(json) {
        ...
        try {
            ...
            return `Ошибка при преобразовании в YAML: ${e.message}`;
        }
    }
});
