document.addEventListener('DOMContentLoaded', () => {
    ...
    const processJson = (isInitialLoad = false) => {
        ...
        try {
            ...
            currentJson = null;
            updateStatus(`JSON non valido: ${e.message}`, true);
            for (const key in outputs) {
                if (outputs[key]) {
                    if (key === 'tree') outputs[key].innerHTML = '';
                    else outputs[key].textContent = '';
    ...
    const renderOutputs = () => {
        ...
        try {
            ...
            outputs.base64.textContent = 'Errore durante la codifica in Base64.';
        }
    ...
    copyBtn.addEventListener('click', () => {
        if (currentJson === null) {
            updateStatus('Niente da copiare.', true);
            return;
        }
        ...
        if (contentToCopy) {
            ...
            }, () => updateStatus('Copia non riuscita.', true));
        }
    });
    ...
    function jsonToXml(json) {
        ...
        try {
            ...
            return `Errore durante la conversione in XML: ${e.message}`;
        }
    }
    ...
    function jsonToYaml(json) {
        ...
        try {
            ...
            return `Errore durante la conversione in YAML: ${e.message}`;
        }
    }
});
