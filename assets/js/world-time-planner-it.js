document.addEventListener('DOMContentLoaded', () => {
    ...
    const countryTimeInfo = {
        'USA': { description: 'Gli Stati Uniti utilizzano 4 fusi orari: <strong>Eastern Time (EST/EDT)</strong>, <strong>Central Time (CST/CDT)</strong>, <strong>Mountain Time (MST/MDT)</strong> e <strong>Pacific Time (PST/PDT)</strong>. Osserva l\'ora legale.' },
        'Canada': { description: 'Il Canada utilizza 1 fuso orario: <strong>Eastern Time (EST/EDT)</strong> e osserva l\'ora legale.' },
        'Brazil': { description: 'Il Brasile utilizza 1 fuso orario: <strong>Brasília Time (BRT)</strong> e non osserva l\'ora legale.' },
        'UK': { description: 'Il Regno Unito utilizza 1 fuso orario: <strong>Greenwich Mean Time (GMT)</strong> e <strong>British Summer Time (BST)</strong>, osservando l\'ora legale.' },
        'France': { description: 'La Francia utilizza 1 fuso orario: <strong>Central European Time (CET)</strong> e <strong>Central European Summer Time (CEST)</strong>, osservando l\'ora legale.' },
        'Germany': { description: 'La Germania utilizza 1 fuso orario: <strong>Central European Time (CET)</strong> e <strong>Central European Summer Time (CEST)</strong>, osservando l\'ora legale.' },
        'Russia': { description: 'La Russia utilizza 1 fuso orario: <strong>Moscow Standard Time (MSK)</strong> e non osserva l\'ora legale.' },
        'Egypt': { description: 'L\'Egitto utilizza 1 fuso orario: <strong>Eastern European Time (EET)</strong> e <strong>Eastern European Summer Time (EEST)</strong>, osservando l\'ora legale.' },
        'South Africa': { description: 'Il Sudafrica utilizza 1 fuso orario: <strong>South African Standard Time (SAST)</strong> e non osserva l\'ora legale.' },
        'UAE': { description: 'Gli Emirati Arabi Uniti utilizzano 1 fuso orario: <strong>Gulf Standard Time (GST)</strong> e non osservano l\'ora legale.' },
        'India': { description: 'L\'India utiliza 1 fuso orario: <strong>Indian Standard Time (IST)</strong> e non osserva l\'ora legale.' },
        'Singapore': { description: 'Singapore utilizza 1 fuso orario: <strong>Singapore Standard Time (SGT)</strong> e non osserva l\'ora legale.' },
        'China': { description: 'La Cina utilizza 1 fuso orario: <strong>China Standard Time (CST)</strong> e non osserva l\'ora legale. L\'intero paese utilizza l\'ora di Pechino.' },
        'South Korea': { description: 'La Corea del Sud utilizza 1 fuso orario: <strong>Korea Standard Time (KST)</strong> e non osserva l\'ora legale.' },
        'Japan': { description: 'Il Giappone utilizza 1 fuso orario: <strong>Japan Standard Time (JST)</strong> e non osserva l\'ora legale.' },
        'Australia': { description: 'L\'Australia utilizza 1 fuso orario: <strong>Australian Eastern Standard Time (AEST)</strong> e <strong>Australian Eastern Daylight Time (AEDT)</strong>, osservando l\'ora legale.' },
        'New Zealand': { description: 'La Nuova Zelanda utilizza 1 fuso orario: <strong>New Zealand Standard Time (NZST)</strong> e <strong>New Zealand Daylight Time (NZDT)</strong>, osservando l\'ora legale.' }
    };
    ...
    function getLocalCityName(timezone) {
        const timezoneMap = {
            'America/New_York': 'New York', 'America/Chicago': 'Chicago', 'America/Denver': 'Denver', 'America/Los_Angeles': 'Los Angeles', 'Europe/London': 'Londra', 'Europe/Paris': 'Parigi', 'Europe/Berlin': 'Berlino', 'Asia/Tokyo': 'Tokyo', 'Asia/Shanghai': 'Shanghai', 'Asia/Hong_Kong': 'Hong Kong', 'Asia/Singapore': 'Singapore', 'Australia/Sydney': 'Sydney', 'Australia/Melbourne': 'Melbourne', 'Pacific/Auckland': 'Auckland', 'America/Toronto': 'Toronto', 'America/Vancouver': 'Vancouver', 'Europe/Madrid': 'Madrid', 'Europe/Rome': 'Roma', 'Asia/Seoul': 'Seoul', 'Asia/Mumbai': 'Mumbai', 'America/Sao_Paulo': 'San Paolo', 'America/Mexico_City': 'Città del Messico', 'Africa/Cairo': 'Il Cairo', 'Africa/Johannesburg': 'Johannesburg'
        };
    ...
    function handleRowsMouseMove(percent, selectorLeft) {
        ...
        rows.forEach(row => {
            ...
            const timeFormatter = new Intl.DateTimeFormat('it-IT', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
    ...
    function loadCityOrder() {
        try {
            ...
            console.warn('Impossibile caricare l\'ordine delle città:', e);
        }
        return null;
    }
    ...
    function updateSingleRowTimeDisplay(row) {
        ...
        try {
            const timeFormatter = new Intl.DateTimeFormat('it-IT', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
            const dateFormatter = new Intl.DateTimeFormat('it-IT', { timeZone: timezone, weekday: 'short', month: 'short', day: 'numeric' });
            const timeString = timeFormatter.format(now);
            const dateString = dateFormatter.format(now);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Ora locale corrente';
            timeElement.innerHTML = `${timeString}<br><span style="font-size: 0.8em; color: #aaa;">${dateString}</span>`;
        } catch (error) {
            console.warn(`Impossibile formattare l\'ora per il fuso orario ${timezone}:`, error);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Fuso orario non valido';
            timeElement.innerHTML = `Non valido<br><span style="font-size: 0.8em; color: #aaa;">Fuso orario</span>`;
        }
    }
    ...
    function renderTimelineGrid(row) {
        ...
        const hourFormatter = new Intl.DateTimeFormat('it-IT', { timeZone: timezone, hour: 'numeric', hour12: false });
        const fullDateFormatter = new Intl.DateTimeFormat('it-IT', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
        const weekdayFormatter = new Intl.DateTimeFormat('it-IT', { timeZone: timezone, weekday: 'short' });
        const ymdPartsFormatter = new Intl.DateTimeFormat('it-IT', { timeZone: timezone, year: 'numeric', month: '2-digit', day: '2-digit' });
        const now = new Date();
        ...
        for (let i = 0; i < TIMELINE_HOURS; i++) {
            ...
            if (weekdayShort === 'sab' || weekdayShort === 'dom') {
            ...
            if (dateOfThisBlock === currentDateInTimezone && localHour === currentHourInTimezone) {
                ...
                currentTimeIndicator.textContent = 'ADESSO';
            ...
            if (hourCount >= 2) {
                ...
                if (hourBlocks.length > 0) {
                    ...
                    if (startIndex >= 0) {
                        ...
                        const timezone = row.dataset.timezone;
                        const timezoneWeekdayFormatter = new Intl.DateTimeFormat('it-IT', { timeZone: timezone, weekday: 'short' });
                        const timezoneDateFormatter = new Intl.DateTimeFormat('it-IT', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
    ...
    function handleMobileTouch(percent) {
        ...
        const endPercent = Math.min(100, percent + (60 / (24 * 60)) * 100); // Assuming a 60-minute selection for touch
    ...
    function createMobileRangeOverlay(startPercent, endPercent) {
        ...
        button.textContent = 'Visualizza Orari';
    ...
    function updateTimezoneTimesDisplay(timezoneTimesData) {
        ...
        timezoneTimesData.forEach(data => {
            ...
            const startFormatted = start.toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' });
            const endFormatted = end.toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' });
            const startDateFormatted = start.toLocaleDateString('it-IT', { weekday: 'short', month: 'short', day: 'numeric' });
    ...
    document.getElementById('wtp-google-meeting-btn').addEventListener('click', () => {
        ...
        const title = 'Riunione programmata';
        const details = 'Dettagli della riunione';
    ...
    document.getElementById('wtp-outlook-meeting-btn').addEventListener('click', () => {
        ...
        const { start, end } = currentTimeRange;
        const title = 'Riunione programmata';
        const url = `https://outlook.live.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(title)}&startdt=${start.toISOString()}&enddt=${end.toISOString()}&body=${encodeURIComponent('Dettagli della riunione')}`;
        window.open(url, '_blank');
    });
