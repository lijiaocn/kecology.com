document.addEventListener('DOMContentLoaded', () => {
    ...
    const countryTimeInfo = {
        'USA': { description: 'Os Estados Unidos usam 4 fusos horários: <strong>Horário do Leste (EST/EDT)</strong>, <strong>Horário Central (CST/CDT)</strong>, <strong>Horário das Montanhas (MST/MDT)</strong> e <strong>Horário do Pacífico (PST/PDT)</strong>. Observa o horário de verão.' },
        'Canada': { description: 'O Canadá usa 1 fuso horário: <strong>Horário do Leste (EST/EDT)</strong> e observa o horário de verão.' },
        'Brazil': { description: 'O Brasil usa 1 fuso horário: <strong>Horário de Brasília (BRT)</strong> e não observa o horário de verão.' },
        'UK': { description: 'O Reino Unido usa 1 fuso horário: <strong>Horário de Greenwich (GMT)</strong> e <strong>Horário de Verão Britânico (BST)</strong>, observando o horário de verão.' },
        'France': { description: 'A França usa 1 fuso horário: <strong>Horário da Europa Central (CET)</strong> e <strong>Horário de Verão da Europa Central (CEST)</strong>, observando o horário de verão.' },
        'Germany': { description: 'A Alemanha usa 1 fuso horário: <strong>Horário da Europa Central (CET)</strong> e <strong>Horário de Verão da Europa Central (CEST)</strong>, observando o horário de verão.' },
        'Russia': { description: 'A Rússia usa 1 fuso horário: <strong>Horário Padrão de Moscou (MSK)</strong> e não observa o horário de verão.' },
        'Egypt': { description: 'O Egito usa 1 fuso horário: <strong>Horário da Europa Oriental (EET)</strong> e <strong>Horário de Verão da Europa Oriental (EEST)</strong>, observando o horário de verão.' },
        'South Africa': { description: 'A África do Sul usa 1 fuso horário: <strong>Horário Padrão da África do Sul (SAST)</strong> e não observa o horário de verão.' },
        'UAE': { description: 'Os Emirados Árabes Unidos usam 1 fuso horário: <strong>Horário Padrão do Golfo (GST)</strong> e não observam o horário de verão.' },
        'India': { description: 'A Índia usa 1 fuso horário: <strong>Horário Padrão da Índia (IST)</strong> e não observa o horário de verão.' },
        'Singapore': { description: 'Singapura usa 1 fuso horário: <strong>Horário Padrão de Singapura (SGT)</strong> e não observa o horário de verão.' },
        'China': { description: 'A China usa 1 fuso horário: <strong>Horário Padrão da China (CST)</strong> e não observa o horário de verão. O país inteiro usa o horário de Pequim.' },
        'South Korea': { description: 'A Coreia do Sul usa 1 fuso horário: <strong>Horário Padrão da Coreia (KST)</strong> e não observa o horário de verão.' },
        'Japan': { description: 'O Japão usa 1 fuso horário: <strong>Horário Padrão do Japão (JST)</strong> e não observa o horário de verão.' },
        'Australia': { description: 'A Austrália usa 1 fuso horário: <strong>Horário Padrão do Leste Australiano (AEST)</strong> e <strong>Horário de Verão do Leste Australiano (AEDT)</strong>, observando o horário de verão.' },
        'New Zealand': { description: 'A Nova Zelândia usa 1 fuso horário: <strong>Horário Padrão da Nova Zelândia (NZST)</strong> e <strong>Horário de Verão da Nova Zelândia (NZDT)</strong>, observando o horário de verão.' }
    };
    ...
    function getLocalCityName(timezone) {
        const timezoneMap = {
            'America/New_York': 'Nova Iorque', 'America/Chicago': 'Chicago', 'America/Denver': 'Denver', 'America/Los_Angeles': 'Los Angeles', 'Europe/London': 'Londres', 'Europe/Paris': 'Paris', 'Europe/Berlin': 'Berlim', 'Asia/Tokyo': 'Tóquio', 'Asia/Shanghai': 'Xangai', 'Asia/Hong_Kong': 'Hong Kong', 'Asia/Singapore': 'Singapura', 'Australia/Sydney': 'Sydney', 'Australia/Melbourne': 'Melbourne', 'Pacific/Auckland': 'Auckland', 'America/Toronto': 'Toronto', 'America/Vancouver': 'Vancouver', 'Europe/Madrid': 'Madrid', 'Europe/Rome': 'Roma', 'Asia/Seoul': 'Seul', 'Asia/Mumbai': 'Mumbai', 'America/Sao_Paulo': 'São Paulo', 'America/Mexico_City': 'Cidade do México', 'Africa/Cairo': 'Cairo', 'Africa/Johannesburg': 'Joanesburgo'
        };
    ...
    function handleRowsMouseMove(percent, selectorLeft) {
        ...
        rows.forEach(row => {
            ...
            const timeFormatter = new Intl.DateTimeFormat('pt-BR', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
    ...
    function loadCityOrder() {
        try {
            ...
            console.warn('Falha ao carregar a ordem da cidade:', e);
        }
        return null;
    }
    ...
    function updateSingleRowTimeDisplay(row) {
        ...
        try {
            const timeFormatter = new Intl.DateTimeFormat('pt-BR', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
            const dateFormatter = new Intl.DateTimeFormat('pt-BR', { timeZone: timezone, weekday: 'short', month: 'short', day: 'numeric' });
            const timeString = timeFormatter.format(now);
            const dateString = dateFormatter.format(now);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Hora local atual';
            timeElement.innerHTML = `${timeString}<br><span style="font-size: 0.8em; color: #aaa;">${dateString}</span>`;
        } catch (error) {
            console.warn(`Falha ao formatar a hora para o fuso horário ${timezone}:`, error);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Fuso horário inválido';
            timeElement.innerHTML = `Inválido<br><span style="font-size: 0.8em; color: #aaa;">Fuso horário</span>`;
        }
    }
    ...
    function renderTimelineGrid(row) {
        ...
        const hourFormatter = new Intl.DateTimeFormat('pt-BR', { timeZone: timezone, hour: 'numeric', hour12: false });
        const fullDateFormatter = new Intl.DateTimeFormat('pt-BR', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
        const weekdayFormatter = new Intl.DateTimeFormat('pt-BR', { timeZone: timezone, weekday: 'short' });
        const ymdPartsFormatter = new Intl.DateTimeFormat('pt-BR', { timeZone: timezone, year: 'numeric', month: '2-digit', day: '2-digit' });
        const now = new Date();
        ...
        for (let i = 0; i < TIMELINE_HOURS; i++) {
            ...
            if (dateOfThisBlock === currentDateInTimezone && localHour === currentHourInTimezone) {
                ...
                currentTimeIndicator.textContent = 'AGORA';
            ...
            if (hourCount >= 2) {
                ...
                if (hourBlocks.length > 0) {
                    ...
                    if (startIndex >= 0) {
                        ...
                        const timezone = row.dataset.timezone;
                        const timezoneWeekdayFormatter = new Intl.DateTimeFormat('pt-BR', { timeZone: timezone, weekday: 'short' });
                        const timezoneDateFormatter = new Intl.DateTimeFormat('pt-BR', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
    ...
    function updateTimezoneTimesDisplay(timezoneTimesData) {
        ...
        timezoneTimesData.forEach(data => {
            ...
            const startDateFormatted = start.toLocaleDateString('pt-BR', { weekday: 'short', month: 'short', day: 'numeric' });
    ...
    document.getElementById('wtp-google-meeting-btn').addEventListener('click', () => {
        ...
        const title = 'Reunião Agendada';
        const details = 'Detalhes da reunião';
    ...
    document.getElementById('wtp-outlook-meeting-btn').addEventListener('click', () => {
        ...
        const { start, end } = currentTimeRange;
        const title = 'Reunião Agendada';
        const url = `https://outlook.live.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(title)}&startdt=${start.toISOString()}&enddt=${end.toISOString()}&body=${encodeURIComponent('Detalhes da reunião')}`;
        window.open(url, '_blank');
    });
