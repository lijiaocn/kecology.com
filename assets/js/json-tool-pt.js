document.addEventListener('DOMContentLoaded', () => {
    ...
    const processJson = (isInitialLoad = false) => {
        ...
        try {
            ...
            updateStatus('JSON válido', false);
            renderOutputs();
            localStorage.setItem(STORAGE_KEY, jsonString);
        } catch (e) {
            currentJson = null;
            updateStatus(`JSON inválido: ${e.message}`, true);
            for (const key in outputs) {
                if (outputs[key]) {
                    if (key === 'tree') outputs[key].innerHTML = '';
                    else outputs[key].textContent = '';
    ...
    const renderOutputs = () => {
        ...
        try {
            ...
            outputs.base64.textContent = 'Erro ao codificar para Base64.';
        }
    ...
    copyBtn.addEventListener('click', () => {
        if (currentJson === null) {
            updateStatus('Nada para copiar.', true);
            return;
        }
        ...
        if (contentToCopy) {
            ...
            }, () => updateStatus('Falha ao copiar.', true));
        }
    });
    ...
    function jsonToXml(json) {
        ...
        try {
            ...
            return `Erro ao converter para XML: ${e.message}`;
        }
    }
    ...
    function jsonToYaml(json) {
        ...
        try {
            ...
            return `Erro ao converter para YAML: ${e.message}`;
        }
    }
});
