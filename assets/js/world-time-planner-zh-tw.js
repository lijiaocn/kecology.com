document.addEventListener('DOMContentLoaded', () => {
    ...
    const countryTimeInfo = {
        'USA': { description: '美國使用4個時區：<strong>東部時間(EST/EDT)</strong>、<strong>中部時間(CST/CDT)</strong>、<strong>山區時間(MST/MDT)</strong>和<strong>太平洋時間(PST/PDT)</strong>。實行夏令時。' },
        'Canada': { description: '加拿大使用1個時區：<strong>東部時間(EST/EDT)</strong>並實行夏令時。' },
        'Brazil': { description: '巴西使用1個時區：<strong>巴西利亞時間(BRT)</strong>，不實行夏令時。' },
        'UK': { description: '英國使用1個時區：<strong>格林威治標準時間(GMT)</strong>和<strong>英國夏令時(BST)</strong>，實行夏令時。' },
        'France': { description: '法國使用1個時區：<strong>中歐時間(CET)</strong>和<strong>中歐夏令時(CEST)</strong>，實行夏令時。' },
        'Germany': { description: '德國使用1個時區：<strong>中歐時間(CET)</strong>和<strong>中歐夏令時(CEST)</strong>，實行夏令時。' },
        'Russia': { description: '俄羅斯使用1個時區：<strong>莫斯科標準時間(MSK)</strong>，不实行夏令时。' },
        'Egypt': { description: '埃及使用1個時區：<strong>東歐時間(EET)</strong>和<strong>東歐夏令時(EEST)</strong>，實行夏令時。' },
        'South Africa': { description: '南非使用1個時區：<strong>南非標準時間(SAST)</strong>，不實行夏令時。' },
        'UAE': { description: '阿聯酋使用1個時區：<strong>海灣標準時間(GST)</strong>，不實行夏令時。' },
        'India': { description: '印度使用1個時區：<strong>印度標準時間(IST)</strong>，不實行夏令時。' },
        'Singapore': { description: '新加坡使用1個時區：<strong>新加坡標準時間(SGT)</strong>，不實行夏令時。' },
        'China': { description: '中國使用1個時區：<strong>中國標準時間(CST)</strong>，不實行夏令時。全國統一使用北京時間。' },
        'South Korea': { description: '韓國使用1個時區：<strong>韓國標準時間(KST)</strong>，不實行夏令時。' },
        'Japan': { description: '日本使用1個時區：<strong>日本標準時間(JST)</strong>，不實行夏令時。' },
        'Australia': { description: '澳洲使用1個時區：<strong>澳洲東部標準時間(AEST)</strong>和<strong>澳洲東部夏令時(AEDT)</strong>，實行夏令時。' },
        'New Zealand': { description: '紐西蘭使用1個時區：<strong>紐西蘭標準時間(NZST)</strong>和<strong>紐西蘭夏令時(NZDT)</strong>，實行夏令時。' }
    };
    ...
    function getLocalCityName(timezone) {
        const timezoneMap = {
            'America/New_York': '紐約', 'America/Chicago': '芝加哥', 'America/Denver': '丹佛', 'America/Los_Angeles': '洛杉磯', 'Europe/London': '倫敦', 'Europe/Paris': '巴黎', 'Europe/Berlin': '柏林', 'Asia/Tokyo': '東京', 'Asia/Shanghai': '上海', 'Asia/Hong_Kong': '香港', 'Asia/Singapore': '新加坡', 'Australia/Sydney': '雪梨', 'Australia/Melbourne': '墨爾本', 'Pacific/Auckland': '奧克蘭', 'America/Toronto': '多倫多', 'America/Vancouver': '溫哥華', 'Europe/Madrid': '馬德里', 'Europe/Rome': '羅馬', 'Asia/Seoul': '首爾', 'Asia/Mumbai': '孟買', 'America/Sao_Paulo': '聖保羅', 'America/Mexico_City': '墨西哥城', 'Africa/Cairo': '開羅', 'Africa/Johannesburg': '約翰尼斯堡'
        };
    ...
    function handleRowsMouseMove(percent, selectorLeft) {
        ...
        rows.forEach(row => {
            ...
            const timeFormatter = new Intl.DateTimeFormat('zh-TW', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
    ...
    function updateSingleRowTimeDisplay(row) {
        ...
        try {
            const timeFormatter = new Intl.DateTimeFormat('zh-TW', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
            const dateFormatter = new Intl.DateTimeFormat('zh-TW', { timeZone: timezone, weekday: 'short', month: 'short', day: 'numeric' });
            const timeString = timeFormatter.format(now);
            const dateString = dateFormatter.format(now);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = '目前本地時間';
            timeElement.innerHTML = `${timeString}<br><span style="font-size: 0.8em; color: #aaa;">${dateString}</span>`;
        } catch (error) {
            console.warn(`無法格式化時區 ${timezone} 的時間:`, error);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = '無效時區';
            timeElement.innerHTML = `無效<br><span style="font-size: 0.8em; color: #aaa;">時區</span>`;
        }
    }
    ...
    function renderTimelineGrid(row) {
        ...
        const hourFormatter = new Intl.DateTimeFormat('zh-TW', { timeZone: timezone, hour: 'numeric', hour12: false });
        const fullDateFormatter = new Intl.DateTimeFormat('zh-TW', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
        const weekdayFormatter = new Intl.DateTimeFormat('zh-TW', { timeZone: timezone, weekday: 'short' });
        const ymdPartsFormatter = new Intl.DateTimeFormat('zh-TW', { timeZone: timezone, year: 'numeric', month: '2-digit', day: '2-digit' });
        const now = new Date();
        ...
        for (let i = 0; i < TIMELINE_HOURS; i++) {
            ...
            if (weekdayShort === '週六' || weekdayShort === '週日') {
            ...
            if (dateOfThisBlock === currentDateInTimezone && localHour === currentHourInTimezone) {
                ...
                currentTimeIndicator.textContent = '現在';
            ...
            if (hourCount >= 2) {
                ...
                if (hourBlocks.length > 0) {
                    ...
                    if (startIndex >= 0) {
                        ...
                        const timezone = row.dataset.timezone;
                        const timezoneWeekdayFormatter = new Intl.DateTimeFormat('zh-TW', { timeZone: timezone, weekday: 'short' });
                        const timezoneDateFormatter = new Intl.DateTimeFormat('zh-TW', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
    ...
    function formatTimeInTimezone(date, timezone) {
        try {
            const formatter = new Intl.DateTimeFormat('zh-TW', { timeZone: timezone, year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false });
    ...
    function updateRangeInfo(timeRange) {
        ...
        document.getElementById('wtp-duration-display').textContent = `${durationHours}小時 ${durationMinutes}分鐘`;
    }
    ...
    function updateTimezoneTimesDisplay(timezoneTimesData) {
        ...
        timezoneTimesData.forEach(data => {
            ...
            const startFormatted = start.toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit' });
            const endFormatted = end.toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit' });
            const startDateFormatted = start.toLocaleDateString('zh-TW', { weekday: 'short', month: 'short', day: 'numeric' });
    ...
    document.getElementById('wtp-outlook-meeting-btn').addEventListener('click', () => {
        ...
        const { start, end } = currentTimeRange;
        const title = '預定會議';
        const url = `https://outlook.live.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(title)}&startdt=${start.toISOString()}&enddt=${end.toISOString()}&body=${encodeURIComponent('會議詳情')}`;
        window.open(url, '_blank');
    });
