document.addEventListener('DOMContentLoaded', () => {
    ...
    const processJson = (isInitialLoad = false) => {
        ...
        try {
            ...
            updateStatus('JSON valide', false);
            renderOutputs();
            localStorage.setItem(STORAGE_KEY, jsonString);
        } catch (e) {
            currentJson = null;
            updateStatus(`JSON invalide: ${e.message}`, true);
            for (const key in outputs) {
                if (outputs[key]) {
                    if (key === 'tree') outputs[key].innerHTML = '';
                    else outputs[key].textContent = '';
    ...
    const renderOutputs = () => {
        ...
        try {
            ...
            outputs.base64.textContent = 'Erreur lors de l\'encodage en Base64.';
        }
    ...
    copyBtn.addEventListener('click', () => {
        if (currentJson === null) {
            updateStatus('Rien à copier.', true);
            return;
        }
        ...
        if (contentToCopy) {
            ...
            }, () => updateStatus('Échec de la copie.', true));
        }
    });
