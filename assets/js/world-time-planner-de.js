document.addEventListener('DOMContentLoaded', () => {
    ...
    const countryTimeInfo = {
        'USA': { description: 'Die Vereinigten Staaten verwenden 4 Zeitzonen: <strong>Eastern Time (EST/EDT)</strong>, <strong>Central Time (CST/CDT)</strong>, <strong>Mountain Time (MST/MDT)</strong> und <strong>Pacific Time (PST/PDT)</strong>. Sommerzeit wird beachtet.' },
        'Canada': { description: 'Kanada verwendet 1 Zeitzone: <strong>Eastern Time (EST/EDT)</strong> und beachtet die Sommerzeit.' },
        'Brazil': { description: 'Brasilien verwendet 1 Zeitzone: <strong>Brasília Time (BRT)</strong> und beachtet keine Sommerzeit.' },
        'UK': { description: 'Das Vereinigte Königreich verwendet 1 Zeitzone: <strong>Greenwich Mean Time (GMT)</strong> und <strong>British Summer Time (BST)</strong>, wobei die Sommerzeit beachtet wird.' },
        'France': { description: 'Frankreich verwendet 1 Zeitzone: <strong>Mitteleuropäische Zeit (MEZ)</strong> und <strong>Mitteleuropäische Sommerzeit (MESZ)</strong>, wobei die Sommerzeit beachtet wird.' },
        'Germany': { description: 'Deutschland verwendet 1 Zeitzone: <strong>Mitteleuropäische Zeit (MEZ)</strong> und <strong>Mitteleuropäische Sommerzeit (MESZ)</strong>, wobei die Sommerzeit beachtet wird.' },
        'Russia': { description: 'Russland verwendet 1 Zeitzone: <strong>Moskauer Standardzeit (MSK)</strong> und beachtet keine Sommerzeit.' },
        'Egypt': { description: 'Ägypten verwendet 1 Zeitzone: <strong>Osteuropäische Zeit (OEZ)</strong> und <strong>Osteuropäische Sommerzeit (OESZ)</strong>, wobei die Sommerzeit beachtet wird.' },
        'South Africa': { description: 'Südafrika verwendet 1 Zeitzone: <strong>Südafrikanische Standardzeit (SAST)</strong> und beachtet keine Sommerzeit.' },
        'UAE': { description: 'Die VAE verwenden 1 Zeitzone: <strong>Golf-Standardzeit (GST)</strong> und beachten keine Sommerzeit.' },
        'India': { description: 'Indien verwendet 1 Zeitzone: <strong>Indische Standardzeit (IST)</strong> und beachtet keine Sommerzeit.' },
        'Singapore': { description: 'Singapur verwendet 1 Zeitzone: <strong>Singapur-Standardzeit (SGT)</strong> und beachtet keine Sommerzeit.' },
        'China': { description: 'China verwendet 1 Zeitzone: <strong>Chinesische Standardzeit (CST)</strong> und beachtet keine Sommerzeit. Das ganze Land verwendet die Pekinger Zeit.' },
        'South Korea': { description: 'Südkorea verwendet 1 Zeitzone: <strong>Koreanische Standardzeit (KST)</strong> und beachtet keine Sommerzeit.' },
        'Japan': { description: 'Japan verwendet 1 Zeitzone: <strong>Japanische Standardzeit (JST)</strong> und beachtet keine Sommerzeit.' },
        'Australia': { description: 'Australien verwendet 1 Zeitzone: <strong>Australische Ostdardzeit (AEST)</strong> und <strong>Australische Ost-Sommerzeit (AEDT)</strong>, wobei die Sommerzeit beachtet wird.' },
        'New Zealand': { description: 'Neuseeland verwendet 1 Zeitzone: <strong>Neuseeländische Standardzeit (NZST)</strong> und <strong>Neuseeländische Sommerzeit (NZDT)</strong>, wobei die Sommerzeit beachtet wird.' }
    };
    ...
    function getLocalCityName(timezone) {
        const timezoneMap = {
            'America/New_York': 'New York', 'America/Chicago': 'Chicago', 'America/Denver': 'Denver', 'America/Los_Angeles': 'Los Angeles', 'Europe/London': 'London', 'Europe/Paris': 'Paris', 'Europe/Berlin': 'Berlin', 'Asia/Tokyo': 'Tokio', 'Asia/Shanghai': 'Shanghai', 'Asia/Hong_Kong': 'Hongkong', 'Asia/Singapore': 'Singapur', 'Australia/Sydney': 'Sydney', 'Australia/Melbourne': 'Melbourne', 'Pacific/Auckland': 'Auckland', 'America/Toronto': 'Toronto', 'America/Vancouver': 'Vancouver', 'Europe/Madrid': 'Madrid', 'Europe/Rome': 'Rom', 'Asia/Seoul': 'Seoul', 'Asia/Mumbai': 'Mumbai', 'America/Sao_Paulo': 'São Paulo', 'America/Mexico_City': 'Mexiko-Stadt', 'Africa/Cairo': 'Kairo', 'Africa/Johannesburg': 'Johannesburg'
        };
    ...
    function handleRowsMouseMove(percent, selectorLeft) {
        ...
        rows.forEach(row => {
            ...
            const timeFormatter = new Intl.DateTimeFormat('de-DE', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
    ...
    function loadCityOrder() {
        try {
            ...
            console.warn('Fehler beim Laden der Stadtreihenfolge:', e);
        }
        return null;
    }
    ...
    function updateSingleRowTimeDisplay(row) {
        ...
        try {
            const timeFormatter = new Intl.DateTimeFormat('de-DE', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
            const dateFormatter = new Intl.DateTimeFormat('de-DE', { timeZone: timezone, weekday: 'short', month: 'short', day: 'numeric' });
            const timeString = timeFormatter.format(now);
            const dateString = dateFormatter.format(now);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Aktuelle Ortszeit';
            timeElement.innerHTML = `${timeString}<br><span style="font-size: 0.8em; color: #aaa;">${dateString}</span>`;
        } catch (error) {
            console.warn(`Zeitformatierung für Zeitzone ${timezone} fehlgeschlagen:`, error);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Ungültige Zeitzone';
            timeElement.innerHTML = `Ungültig<br><span style="font-size: 0.8em; color: #aaa;">Zeitzone</span>`;
        }
    }
    ...
    function renderTimelineGrid(row) {
        ...
        const hourFormatter = new Intl.DateTimeFormat('de-DE', { timeZone: timezone, hour: 'numeric', hour12: false });
        const fullDateFormatter = new Intl.DateTimeFormat('de-DE', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
        const weekdayFormatter = new Intl.DateTimeFormat('de-DE', { timeZone: timezone, weekday: 'short' });
        const ymdPartsFormatter = new Intl.DateTimeFormat('de-DE', { timeZone: timezone, year: 'numeric', month: '2-digit', day: '2-digit' });
        const now = new Date();
        ...
        for (let i = 0; i < TIMELINE_HOURS; i++) {
            ...
            if (weekdayShort === 'Sa' || weekdayShort === 'So') {
            ...
            if (dateOfThisBlock === currentDateInTimezone && localHour === currentHourInTimezone) {
                ...
                currentTimeIndicator.textContent = 'JETZT';
            ...
            if (hourCount >= 2) {
                ...
                if (hourBlocks.length > 0) {
                    ...
                    if (startIndex >= 0) {
                        ...
                        const timezone = row.dataset.timezone;
                        const timezoneWeekdayFormatter = new Intl.DateTimeFormat('de-DE', { timeZone: timezone, weekday: 'short' });
                        const timezoneDateFormatter = new Intl.DateTimeFormat('de-DE', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
    ...
    function createMobileRangeOverlay(startPercent, endPercent) {
        ...
        button.textContent = 'Zeiten anzeigen';
    ...
    function updateTimezoneTimesDisplay(timezoneTimesData) {
        ...
        timezoneTimesData.forEach(data => {
            ...
            const startFormatted = start.toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' });
            const endFormatted = end.toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' });
            const startDateFormatted = start.toLocaleDateString('de-DE', { weekday: 'short', month: 'short', day: 'numeric' });
    ...
    document.getElementById('wtp-google-meeting-btn').addEventListener('click', () => {
        ...
        const title = 'Geplantes Meeting';
        const details = 'Meeting-Details';
        const url = `https://www.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(title)}&dates=${start.toISOString().replace(/-|:|	heir\d+/g, '')}/${end.toISOString().replace(/-|:|	heir\d+/g, '')}&details=${encodeURIComponent(details)}`;
    ...
    document.getElementById('wtp-outlook-meeting-btn').addEventListener('click', () => {
        ...
        const { start, end } = currentTimeRange;
        const title = 'Geplantes Meeting';
        const url = `https://outlook.live.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(title)}&startdt=${start.toISOString()}&enddt=${end.toISOString()}&body=${encodeURIComponent('Meeting-Details')}`;
        window.open(url, '_blank');
    });
    ...
    document.getElementById('wtp-yahoo-meeting-btn').addEventListener('click', () => {
        ...
        const url = `https://calendar.yahoo.com/?v=60&view=d&type=20&title=${encodeURIComponent(title)}&st=${start.toISOString().replace(/-|:|	heir\d+/g, '')}&dur=${durationHours}${durationMinutes}`;
        window.open(url, '_blank');
    });
