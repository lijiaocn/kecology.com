document.addEventListener('DOMContentLoaded', () => {
    ...
    const countryTimeInfo = {
        'USA': {
            description: 'Les États-Unis utilisent 4 fuseaux horaires : <strong>Heure de l\'Est (EST/EDT)</strong>, <strong>Heure du Centre (CST/CDT)</strong>, <strong>Heure des Rocheuses (MST/MDT)</strong> et <strong>Heure du Pacifique (PST/PDT)</strong>. Applique l\'heure d\'été.'
        },
        'Canada': {
            description: 'Le Canada utilise 1 fuseau horaire : <strong>Heure de l\'Est (EST/EDT)</strong> et applique l\'heure d\'été.'
        },
        'Brazil': {
            description: 'Le Brésil utilise 1 fuseau horaire : <strong>Heure de Brasília (BRT)</strong> et n\'applique pas l\'heure d\'été.'
        },
        'UK': {
            description: 'Le Royaume-Uni utilise 1 fuseau horaire : <strong>Heure de Greenwich (GMT)</strong> et <strong>Heure d\'été britannique (BST)</strong>, appliquant l\'heure d\'été.'
        },
        'France': {
            description: 'La France utilise 1 fuseau horaire : <strong>Heure d\'Europe centrale (CET)</strong> et <strong>Heure d\'été d\'Europe centrale (CEST)</strong>, appliquant l\'heure d\'été.'
        },
        'Germany': {
            description: 'L\'Allemagne utilise 1 fuseau horaire : <strong>Heure d\'Europe centrale (CET)</strong> et <strong>Heure d\'été d\'Europe centrale (CEST)</strong>, appliquant l\'heure d\'été.'
        },
        'Russia': {
            description: 'La Russie utilise 1 fuseau horaire : <strong>Heure de Moscou (MSK)</strong> et n\'applique pas l\'heure d\'été.'
        },
        'Egypt': {
            description: 'L\'Égypte utilise 1 fuseau horaire : <strong>Heure d\'Europe de l\'Est (EET)</strong> et <strong>Heure d\'été d\'Europe de l\'Est (EEST)</strong>, appliquant l\'heure d\'été.'
        },
        'South Africa': {
            description: 'L\'Afrique du Sud utilise 1 fuseau horaire : <strong>Heure normale d\'Afrique du Sud (SAST)</strong> et n\'applique pas l\'heure d\'été.'
        },
        'UAE': {
            description: 'Les Émirats arabes unis utilisent 1 fuseau horaire : <strong>Heure normale du Golfe (GST)</strong> et n\'appliquent pas l\'heure d\'été.'
        },
        'India': {
            description: 'L\'Inde utilise 1 fuseau horaire : <strong>Heure normale de l\'Inde (IST)</strong> et n\'applique pas l\'heure d\'été.'
        },
        'Singapore': {
            description: 'Singapour utilise 1 fuseau horaire : <strong>Heure normale de Singapour (SGT)</strong> et n\'applique pas l\'heure d\'été.'
        },
        'China': {
            description: 'La Chine utilise 1 fuseau horaire : <strong>Heure normale de Chine (CST)</strong> et n\'applique pas l\'heure d\'été. L\'ensemble du pays utilise l\'heure de Pékin.'
        },
        'South Korea': {
            description: 'La Corée du Sud utilise 1 fuseau horaire : <strong>Heure normale de Corée (KST)</strong> et n\'applique pas l\'heure d\'été.'
        },
        'Japan': {
            description: 'Le Japon utilise 1 fuseau horaire : <strong>Heure normale du Japon (JST)</strong> et n\'applique pas l\'heure d\'été.'
        },
        'Australia': {
            description: 'L\'Australie utilise 1 fuseau horaire : <strong>Heure normale de l\'Est de l\'Australie (AEST)</strong> et <strong>Heure d\'été de l\'Est de l\'Australie (AEDT)</strong>, appliquant l\'heure d\'été.'
        },
        'New Zealand': {
            description: 'La Nouvelle-Zélande utilise 1 fuseau horaire : <strong>Heure normale de Nouvelle-Zélande (NZST)</strong> et <strong>Heure d\'été de Nouvelle-Zélande (NZDT)</strong>, appliquant l\'heure d\'été.'
        }
    };
    ...
    function getLocalCityName(timezone) {
        const timezoneMap = {
            'America/New_York': 'New York', 'America/Chicago': 'Chicago', 'America/Denver': 'Denver', 'America/Los_Angeles': 'Los Angeles', 'Europe/London': 'Londres', 'Europe/Paris': 'Paris', 'Europe/Berlin': 'Berlin', 'Asia/Tokyo': 'Tokyo', 'Asia/Shanghai': 'Shanghai', 'Asia/Hong_Kong': 'Hong Kong', 'Asia/Singapore': 'Singapour', 'Australia/Sydney': 'Sydney', 'Australia/Melbourne': 'Melbourne', 'Pacific/Auckland': 'Auckland', 'America/Toronto': 'Toronto', 'America/Vancouver': 'Vancouver', 'Europe/Madrid': 'Madrid', 'Europe/Rome': 'Rome', 'Asia/Seoul': 'Séoul', 'Asia/Mumbai': 'Mumbai', 'America/Sao_Paulo': 'São Paulo', 'America/Mexico_City': 'Mexico', 'Africa/Cairo': 'Le Caire', 'Africa/Johannesburg': 'Johannesburg'
        };
    ...
    function handleRowsMouseMove(percent, selectorLeft) {
        ...
        rows.forEach(row => {
            ...
            const timeFormatter = new Intl.DateTimeFormat('fr-FR', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
    ...
    function loadCityOrder() {
        try {
            ...
            console.warn('Échec du chargement de l\'ordre des villes:', e);
        }
        return null;
    }
    ...
    function updateSingleRowTimeDisplay(row) {
        ...
        try {
            const timeFormatter = new Intl.DateTimeFormat('fr-FR', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
            const dateFormatter = new Intl.DateTimeFormat('fr-FR', { timeZone: timezone, weekday: 'short', month: 'short', day: 'numeric' });
            const timeString = timeFormatter.format(now);
            const dateString = dateFormatter.format(now);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Heure locale actuelle';
            timeElement.innerHTML = `${timeString}<br><span style="font-size: 0.8em; color: #aaa;">${dateString}</span>`;
        } catch (error) {
            console.warn(`Impossible de formater l\'heure pour le fuseau horaire ${timezone}:`, error);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Fuseau horaire invalide';
            timeElement.innerHTML = `Invalide<br><span style="font-size: 0.8em; color: #aaa;">Fuseau horaire</span>`;
        }
    }
    ...
    function renderTimelineGrid(row) {
        ...
        const hourFormatter = new Intl.DateTimeFormat('fr-FR', { timeZone: timezone, hour: 'numeric', hour12: false });
        const fullDateFormatter = new Intl.DateTimeFormat('fr-FR', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
        const weekdayFormatter = new Intl.DateTimeFormat('fr-FR', { timeZone: timezone, weekday: 'short' });
        const ymdPartsFormatter = new Intl.DateTimeFormat('fr-FR', { timeZone: timezone, year: 'numeric', month: '2-digit', day: '2-digit' });
        const now = new Date();
        ...
        for (let i = 0; i < TIMELINE_HOURS; i++) {
            ...
            if (weekdayShort === 'sam.' || weekdayShort === 'dim.') {
            ...
            if (dateOfThisBlock === currentDateInTimezone && localHour === currentHourInTimezone) {
                ...
                currentTimeIndicator.textContent = 'MAINTENANT';
            ...
            if (hourCount >= 2) {
                ...
                if (hourBlocks.length > 0) {
                    ...
                    if (startIndex >= 0) {
                        ...
                        const timezone = row.dataset.timezone;
                        const timezoneWeekdayFormatter = new Intl.DateTimeFormat('fr-FR', { timeZone: timezone, weekday: 'short' });
                        const timezoneDateFormatter = new Intl.DateTimeFormat('fr-FR', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
    ...
    function handleMobileTouch(percent) {
        ...
        const endPercent = Math.min(100, percent + (60 / (24 * 60)) * 100); // Default to 1 hour selection
    ...
    function createMobileRangeOverlay(startPercent, endPercent) {
        ...
        button.textContent = 'Voir les heures';
    ...
    function updateTimezoneTimesDisplay(timezoneTimesData) {
        ...
        timezoneTimesData.forEach(data => {
            ...
            const start = new Date(data.start.replace(/(\d+)\/(\d+)\/(\d+),/, '$3-$1-$2T'));
            const end = new Date(data.end.replace(/(\d+)\/(\d+)\/(\d+),/, '$3-$1-$2T'));
            const startFormatted = start.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
            const endFormatted = end.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
            const startDateFormatted = start.toLocaleDateString('fr-FR', { weekday: 'short', month: 'short', day: 'numeric' });
    ...
    document.getElementById('wtp-google-meeting-btn').addEventListener('click', () => {
        ...
        const title = 'Réunion planifiée';
        const details = 'Détails de la réunion';
    ...
    document.getElementById('wtp-outlook-meeting-btn').addEventListener('click', () => {
        ...
        const { start, end } = currentTimeRange;
        const title = 'Réunion planifiée';
        const url = `https://outlook.live.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(title)}&startdt=${start.toISOString()}&enddt=${end.toISOString()}&body=${encodeURIComponent('Détails de la réunion')}`;
        window.open(url, '_blank');
    });
