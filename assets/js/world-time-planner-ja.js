document.addEventListener('DOMContentLoaded', () => {
    ...
    const countryTimeInfo = {
        'USA': { description: 'アメリカ合衆国は4つのタイムゾーンを使用しています：<strong>東部時間(EST/EDT)</strong>、<strong>中部時間(CST/CDT)</strong>、<strong>山岳部時間(MST/MDT)</strong>、<strong>太平洋時間(PST/PDT)</strong>。夏時間を採用しています。' },
        'Canada': { description: 'カナダは1つのタイムゾーンを使用しています：<strong>東部時間(EST/EDT)</strong>で、夏時間を採用しています。' },
        'Brazil': { description: 'ブラジルは1つのタイムゾーンを使用しています：<strong>ブラジリア時間(BRT)</strong>で、夏時間は採用していません。' },
        'UK': { description: 'イギリスは1つのタイムゾーンを使用しています：<strong>グリニッジ標準時(GMT)</strong>と<strong>英国夏時間(BST)</strong>で、夏時間を採用しています。' },
        'France': { description: 'フランスは1つのタイムゾーンを使用しています：<strong>中央ヨーロッパ時間(CET)</strong>と<strong>中央ヨーロッパ夏時間(CEST)</strong>で、夏時間を採用しています。' },
        'Germany': { description: 'ドイツは1つのタイムゾーンを使用しています：<strong>中央ヨーロッパ時間(CET)</strong>と<strong>中央ヨーロッパ夏時間(CEST)</strong>で、夏時間を採用しています。' },
        'Russia': { description: 'ロシアは1つのタイムゾーンを使用しています：<strong>モスクワ標準時(MSK)</strong>で、夏時間は採用していません。' },
        'Egypt': { description: 'エジプトは1つのタイムゾーンを使用しています：<strong>東ヨーロッパ時間(EET)</strong>と<strong>東ヨーロッパ夏時間(EEST)</strong>で、夏時間を採用しています。' },
        'South Africa': { description: '南アフリカは1つのタイムゾーンを使用しています：<strong>南アフリカ標準時(SAST)</strong>で、夏時間は採用していません。' },
        'UAE': { description: 'アラブ首長国連邦は1つのタイムゾーンを使用しています：<strong>湾岸標準時(GST)</strong>で、夏時間は採用していません。' },
        'India': { description: 'インドは1つのタイムゾーンを使用しています：<strong>インド標準時(IST)</strong>で、夏時間は採用していません。' },
        'Singapore': { description: 'シンガポールは1つのタイムゾーンを使用しています：<strong>シンガポール標準時(SGT)</strong>で、夏時間は採用していません。' },
        'China': { description: '中国は1つのタイムゾーンを使用しています：<strong>中国標準時(CST)</strong>で、夏時間は採用していません。国全体で北京時間を使用しています。' },
        'South Korea': { description: '韓国は1つのタイムゾーンを使用しています：<strong>韓国標準時(KST)</strong>で、夏時間は採用していません。' },
        'Japan': { description: '日本は1つのタイムゾーンを使用しています：<strong>日本標準時(JST)</strong>で、夏時間は採用していません。' },
        'Australia': { description: 'オーストラリアは1つのタイムゾーンを使用しています：<strong>オーストラリア東部標準時(AEST)</strong>と<strong>オーストラリア東部夏時間(AEDT)</strong>で、夏時間を採用しています。' },
        'New Zealand': { description: 'ニュージーランドは1つのタイムゾーンを使用しています：<strong>ニュージーランド標準時(NZST)</strong>と<strong>ニュージーランド夏時間(NZDT)</strong>で、夏時間を採用しています。' }
    };
    ...
    function getLocalCityName(timezone) {
        const timezoneMap = {
            'America/New_York': 'ニューヨーク', 'America/Chicago': 'シカゴ', 'America/Denver': 'デンバー', 'America/Los_Angeles': 'ロサンゼルス', 'Europe/London': 'ロンドン', 'Europe/Paris': 'パリ', 'Europe/Berlin': 'ベルリン', 'Asia/Tokyo': '東京', 'Asia/Shanghai': '上海', 'Asia/Hong_Kong': '香港', 'Asia/Singapore': 'シンガポール', 'Australia/Sydney': 'シドニー', 'Australia/Melbourne': 'メルボルン', 'Pacific/Auckland': 'オークランド', 'America/Toronto': 'トロント', 'America/Vancouver': 'バンクーバー', 'Europe/Madrid': 'マドリード', 'Europe/Rome': 'ローマ', 'Asia/Seoul': 'ソウル', 'Asia/Mumbai': 'ムンバイ', 'America/Sao_Paulo': 'サンパウロ', 'America/Mexico_City': 'メキシコシティ', 'Africa/Cairo': 'カイロ', 'Africa/Johannesburg': 'ヨハネスブルグ'
        };
    ...
    function handleRowsMouseMove(percent, selectorLeft) {
        ...
        rows.forEach(row => {
            ...
            const timeFormatter = new Intl.DateTimeFormat('ja-JP', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
    ...
    function loadCityOrder() {
        try {
            ...
            console.warn('都市の順序を読み込めませんでした:', e);
        }
        return null;
    }
    ...
    function updateSingleRowTimeDisplay(row) {
        ...
        try {
            const timeFormatter = new Intl.DateTimeFormat('ja-JP', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
            const dateFormatter = new Intl.DateTimeFormat('ja-JP', { timeZone: timezone, weekday: 'short', month: 'short', day: 'numeric' });
            const timeString = timeFormatter.format(now);
            const dateString = dateFormatter.format(now);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = '現在の現地時間';
            timeElement.innerHTML = `${timeString}<br><span style="font-size: 0.8em; color: #aaa;">${dateString}</span>`;
        } catch (error) {
            console.warn(`タイムゾーン${timezone}の時刻をフォーマットできませんでした:`, error);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = '無効なタイムゾーン';
            timeElement.innerHTML = `無効<br><span style="font-size: 0.8em; color: #aaa;">タイムゾーン</span>`;
        }
    }
    ...
    function renderTimelineGrid(row) {
        ...
        const hourFormatter = new Intl.DateTimeFormat('ja-JP', { timeZone: timezone, hour: 'numeric', hour12: false });
        const fullDateFormatter = new Intl.DateTimeFormat('ja-JP', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
        const weekdayFormatter = new Intl.DateTimeFormat('ja-JP', { timeZone: timezone, weekday: 'short' });
        const ymdPartsFormatter = new Intl.DateTimeFormat('ja-JP', { timeZone: timezone, year: 'numeric', month: '2-digit', day: '2-digit' });
        const now = new Date();
        ...
        for (let i = 0; i < TIMELINE_HOURS; i++) {
            ...
            if (weekdayShort === '土' || weekdayShort === '日') {
            ...
            if (dateOfThisBlock === currentDateInTimezone && localHour === currentHourInTimezone) {
                ...
                currentTimeIndicator.textContent = '今';
            ...
            if (hourCount >= 2) {
                ...
                if (hourBlocks.length > 0) {
                    ...
                    if (startIndex >= 0) {
                        ...
                        const timezone = row.dataset.timezone;
                        const timezoneWeekdayFormatter = new Intl.DateTimeFormat('ja-JP', { timeZone: timezone, weekday: 'short' });
                        const timezoneDateFormatter = new Intl.DateTimeFormat('ja-JP', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
    ...
    function updateRangeInfo(timeRange) {
        ...
        document.getElementById('wtp-duration-display').textContent = `${durationHours}時間 ${durationMinutes}分`;
    }
    ...
    function updateTimezoneTimesDisplay(timezoneTimesData) {
        ...
        timezoneTimesData.forEach(data => {
            ...
            const start = new Date(data.start.replace(/(\d{4})\/(\d{2})\/(\d{2})\s/, '$1-$2-$3T'));
            const end = new Date(data.end.replace(/(\d{4})\/(\d{2})\/(\d{2})\s/, '$1-$2-$3T'));
            const startFormatted = start.toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' });
            const endFormatted = end.toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' });
            const startDateFormatted = start.toLocaleDateString('ja-JP', { weekday: 'short', month: 'short', day: 'numeric' });
    ...
    document.getElementById('wtp-google-meeting-btn').addEventListener('click', () => {
        ...
        const title = '予定された会議';
        const details = '会議の詳細';
        const url = `https://www.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(title)}&dates=${start.toISOString().replace(/-|:|romUtf8(0x80)/g, '')}/${end.toISOString().replace(/-|:|romUtf8(0x80)/g, '')}&details=${encodeURIComponent(details)}`;
    ...
    document.getElementById('wtp-outlook-meeting-btn').addEventListener('click', () => {
        ...
        const { start, end } = currentTimeRange;
        const title = '予定された会議';
        const url = `https://outlook.live.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(title)}&startdt=${start.toISOString()}&enddt=${end.toISOString()}&body=${encodeURIComponent('会議の詳細')}`;
        window.open(url, '_blank');
    });
    ...
    document.getElementById('wtp-yahoo-meeting-btn').addEventListener('click', () => {
        ...
        const url = `https://calendar.yahoo.com/?v=60&view=d&type=20&title=${encodeURIComponent(title)}&st=${start.toISOString().replace(/-|:|romUtf8(0x80)/g, '')}&dur=${durationHours}${durationMinutes}`;
        window.open(url, '_blank');
    });
