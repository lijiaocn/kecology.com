document.addEventListener('DOMContentLoaded', () => {
    ...
    const countryTimeInfo = {
        'USA': { description: 'Estados Unidos utiliza 4 zonas horarias: <strong>Hora del Este (EST/EDT)</strong>, <strong>Hora del Centro (CST/CDT)</strong>, <strong>Hora de la Montaña (MST/MDT)</strong> y <strong>Hora del Pacífico (PST/PDT)</strong>. Se observa el horario de verano.' },
        'Canada': { description: 'Canadá utiliza 1 zona horaria: <strong>Hora del Este (EST/EDT)</strong> y observa el horario de verano.' },
        'Brazil': { description: 'Brasil utiliza 1 zona horaria: <strong>Hora de Brasilia (BRT)</strong> y no observa el horario de verano.' },
        'UK': { description: 'El Reino Unido utiliza 1 zona horaria: <strong>Hora Media de Greenwich (GMT)</strong> y <strong>Hora de Verano Británica (BST)</strong>, observando el horario de verano.' },
        'France': { description: 'Francia utiliza 1 zona horaria: <strong>Hora de Europa Central (CET)</strong> y <strong>Hora de Verano de Europa Central (CEST)</strong>, observando el horario de verano.' },
        'Germany': { description: 'Alemania utiliza 1 zona horaria: <strong>Hora de Europa Central (CET)</strong> y <strong>Hora de Verano de Europa Central (CEST)</strong>, observando el horario de verano.' },
        'Russia': { description: 'Rusia utiliza 1 zona horaria: <strong>Hora Estándar de Moscú (MSK)</strong> y no observa el horario de verano.' },
        'Egypt': { description: 'Egipto utiliza 1 zona horaria: <strong>Hora de Europa del Este (EET)</strong> y <strong>Hora de Verano de Europa del Este (EEST)</strong>, observando el horario de verano.' },
        'South Africa': { description: 'Sudáfrica utiliza 1 zona horaria: <strong>Hora Estándar de Sudáfrica (SAST)</strong> y no observa el horario de verano.' },
        'UAE': { description: 'Los EAU utilizan 1 zona horaria: <strong>Hora Estándar del Golfo (GST)</strong> y no observan el horario de verano.' },
        'India': { description: 'India utiliza 1 zona horaria: <strong>Hora Estándar de la India (IST)</strong> y no observa el horario de verano.' },
        'Singapore': { description: 'Singapur utiliza 1 zona horaria: <strong>Hora Estándar de Singapur (SGT)</strong> y no observa el horario de verano.' },
        'China': { description: 'China utiliza 1 zona horaria: <strong>Hora Estándar de China (CST)</strong> y no observa el horario de verano. Todo el país utiliza la hora de Pekín.' },
        'South Korea': { description: 'Corea del Sur utiliza 1 zona horaria: <strong>Hora Estándar de Corea (KST)</strong> y no observa el horario de verano.' },
        'Japan': { description: 'Japón utiliza 1 zona horaria: <strong>Hora Estándar de Japón (JST)</strong> y no observa el horario de verano.' },
        'Australia': { description: 'Australia utiliza 1 zona horaria: <strong>Hora Estándar del Este de Australia (AEST)</strong> y <strong>Hora de Verano del Este de Australia (AEDT)</strong>, observando el horario de verano.' },
        'New Zealand': { description: 'Nueva Zelanda utiliza 1 zona horaria: <strong>Hora Estándar de Nueva Zelanda (NZST)</strong> y <strong>Hora de Verano de Nueva Zelanda (NZDT)</strong>, observando el horario de verano.' }
    };
    ...
    function getLocalCityName(timezone) {
        const timezoneMap = {
            'America/New_York': 'Nueva York', 'America/Chicago': 'Chicago', 'America/Denver': 'Denver', 'America/Los_Angeles': 'Los Ángeles', 'Europe/London': 'Londres', 'Europe/Paris': 'París', 'Europe/Berlin': 'Berlín', 'Asia/Tokyo': 'Tokio', 'Asia/Shanghai': 'Shanghái', 'Asia/Hong_Kong': 'Hong Kong', 'Asia/Singapore': 'Singapur', 'Australia/Sydney': 'Sídney', 'Australia/Melbourne': 'Melbourne', 'Pacific/Auckland': 'Auckland', 'America/Toronto': 'Toronto', 'America/Vancouver': 'Vancouver', 'Europe/Madrid': 'Madrid', 'Europe/Rome': 'Roma', 'Asia/Seoul': 'Seúl', 'Asia/Mumbai': 'Mumbai', 'America/Sao_Paulo': 'São Paulo', 'America/Mexico_City': 'Ciudad de México', 'Africa/Cairo': 'El Cairo', 'Africa/Johannesburg': 'Johannesburgo'
        };
    ...
    function handleRowsMouseMove(percent, selectorLeft) {
        ...
        rows.forEach(row => {
            ...
            const timeFormatter = new Intl.DateTimeFormat('es-ES', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
    ...
    function loadCityOrder() {
        try {
            ...
            console.warn('Error al cargar el orden de las ciudades:', e);
        }
        return null;
    }
    ...
    function updateSingleRowTimeDisplay(row) {
        ...
        try {
            const timeFormatter = new Intl.DateTimeFormat('es-ES', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
            const dateFormatter = new Intl.DateTimeFormat('es-ES', { timeZone: timezone, weekday: 'short', month: 'short', day: 'numeric' });
            const timeString = timeFormatter.format(now);
            const dateString = dateFormatter.format(now);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Hora local actual';
            timeElement.innerHTML = `${timeString}<br><span style="font-size: 0.8em; color: #aaa;">${dateString}</span>`;
        } catch (error) {
            console.warn(`No se pudo formatear la hora para la zona horaria ${timezone}:`, error);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Zona horaria no válida';
            timeElement.innerHTML = `Inválido<br><span style="font-size: 0.8em; color: #aaa;">Zona horaria</span>`;
        }
    }
    ...
    function renderTimelineGrid(row) {
        ...
        const hourFormatter = new Intl.DateTimeFormat('es-ES', { timeZone: timezone, hour: 'numeric', hour12: false });
        const fullDateFormatter = new Intl.DateTimeFormat('es-ES', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
        const weekdayFormatter = new Intl.DateTimeFormat('es-ES', { timeZone: timezone, weekday: 'short' });
        const ymdPartsFormatter = new Intl.DateTimeFormat('es-ES', { timeZone: timezone, year: 'numeric', month: '2-digit', day: '2-digit' });
        const now = new Date();
        ...
        for (let i = 0; i < TIMELINE_HOURS; i++) {
            ...
            if (dateOfThisBlock === currentDateInTimezone && localHour === currentHourInTimezone) {
                ...
                currentTimeIndicator.textContent = 'AHORA';
            ...
            if (hourCount >= 2) {
                ...
                if (hourBlocks.length > 0) {
                    ...
                    if (startIndex >= 0) {
                        ...
                        const timezone = row.dataset.timezone;
                        const timezoneWeekdayFormatter = new Intl.DateTimeFormat('es-ES', { timeZone: timezone, weekday: 'short' });
                        const timezoneDateFormatter = new Intl.DateTimeFormat('es-ES', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
    ...
    function handleMobileTouch(percent) {
        ...
        const endPercent = Math.min(100, percent + (60 / (24 * 60)) * 100); // 60 minutes range
    ...
    function updateTimezoneTimesDisplay(timezoneTimesData) {
        ...
        timezoneTimesData.forEach(data => {
            ...
            const endFormatted = end.toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' });
            const startDateFormatted = start.toLocaleDateString('es-ES', { weekday: 'short', month: 'short', day: 'numeric' });
    ...
    document.getElementById('wtp-google-meeting-btn').addEventListener('click', () => {
        ...
        const title = 'Reunión programada';
        const details = 'Detalles de la reunión';
    ...
    document.getElementById('wtp-outlook-meeting-btn').addEventListener('click', () => {
        ...
        const { start, end } = currentTimeRange;
        const title = 'Reunión programada';
        const url = `https://outlook.live.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(title)}&startdt=${start.toISOString()}&enddt=${end.toISOString()}&body=${encodeURIComponent('Detalles de la reunión')}`;
        window.open(url, '_blank');
    });
