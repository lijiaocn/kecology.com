document.addEventListener('DOMContentLoaded', () => {
    ...
    const processJson = (isInitialLoad = false) => {
        ...
        try {
            ...
            updateStatus('JSON yang valid', false);
            renderOutputs();
            localStorage.setItem(STORAGE_KEY, jsonString);
        } catch (e) {
            currentJson = null;
            updateStatus(`JSON tidak valid: ${e.message}`, true);
            for (const key in outputs) {
                if (outputs[key]) {
                    if (key === 'tree') outputs[key].innerHTML = '';
                    else outputs[key].textContent = '';
    ...
    const renderOutputs = () => {
        ...
        try {
            ...
            outputs.base64.textContent = 'Gagal mengodekan ke Base64.';
        }
