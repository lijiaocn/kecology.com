document.addEventListener('DOMContentLoaded', () => {
    ...
    const countryTimeInfo = {
        'USA': { description: 'Amerika Birleşik Devletleri 4 saat dilimi kullanır: <strong>Doğu Saati (EST/EDT)</strong>, <strong>Merkez Saati (CST/CDT)</strong>, <strong>Dağ Saati (MST/MDT)</strong> ve <strong>Pasifik Saati (PST/PDT)</strong>. Yaz saati uygulaması kullanılır.' },
        'Canada': { description: 'Kanada 1 saat dilimi kullanır: <strong>Doğu Saati (EST/EDT)</strong> ve yaz saati uygulaması kullanılır.' },
        'Brazil': { description: 'Brezilya 1 saat dilimi kullanır: <strong>Brasília Saati (BRT)</strong> ve yaz saati uygulaması kullanılmaz.' },
        'UK': { description: 'Birleşik Krallık 1 saat dilimi kullanır: <strong>Greenwich Ortalama Saati (GMT)</strong> ve <strong>İngiliz Yaz Saati (BST)</strong>, yaz saati uygulaması kullanılır.' },
        'France': { description: 'Fransa 1 saat dilimi kullanır: <strong>Orta Avrupa Saati (CET)</strong> ve <strong>Orta Avrupa Yaz Saati (CEST)</strong>, yaz saati uygulaması kullanılır.' },
        'Germany': { description: 'Almanya 1 saat dilimi kullanır: <strong>Orta Avrupa Saati (CET)</strong> ve <strong>Orta Avrupa Yaz Saati (CEST)</strong>, yaz saati uygulaması kullanılır.' },
        'Russia': { description: 'Rusya 1 saat dilimi kullanır: <strong>Moskova Standart Saati (MSK)</strong> ve yaz saati uygulaması kullanılmaz.' },
        'Egypt': { description: 'Mısır 1 saat dilimi kullanır: <strong>Doğu Avrupa Saati (EET)</strong> ve <strong>Doğu Avrupa Yaz Saati (EEST)</strong>, yaz saati uygulaması kullanılır.' },
        'South Africa': { description: 'Güney Afrika 1 saat dilimi kullanır: <strong>Güney Afrika Standart Saati (SAST)</strong> ve yaz saati uygulaması kullanılmaz.' },
        'UAE': { description: 'BAE 1 saat dilimi kullanır: <strong>Körfez Standart Saati (GST)</strong> ve yaz saati uygulaması kullanılmaz.' },
        'India': { description: 'Hindistan 1 saat dilimi kullanır: <strong>Hindistan Standart Saati (IST)</strong> ve yaz saati uygulaması kullanılmaz.' },
        'Singapore': { description: 'Singapur 1 saat dilimi kullanır: <strong>Singapur Standart Saati (SGT)</strong> ve yaz saati uygulaması kullanılmaz.' },
        'China': { description: 'Çin 1 saat dilimi kullanır: <strong>Çin Standart Saati (CST)</strong> ve yaz saati uygulaması kullanılmaz. Bütün ülke Pekin saatini kullanır.' },
        'South Korea': { description: 'Güney Kore 1 saat dilimi kullanır: <strong>Kore Standart Saati (KST)</strong> ve yaz saati uygulaması kullanılmaz.' },
        'Japan': { description: 'Japonya 1 saat dilimi kullanır: <strong>Japonya Standart Saati (JST)</strong> ve yaz saati uygulaması kullanılmaz.' },
        'Australia': { description: 'Avustralya 1 saat dilimi kullanır: <strong>Avustralya Doğu Standart Saati (AEST)</strong> ve <strong>Avustralya Doğu Yaz Saati (AEDT)</strong>, yaz saati uygulaması kullanılır.' },
        'New Zealand': { description: 'Yeni Zelanda 1 saat dilimi kullanır: <strong>Yeni Zelanda Standart Saati (NZST)</strong> ve <strong>Yeni Zelanda Yaz Saati (NZDT)</strong>, yaz saati uygulaması kullanılır.' }
    };
    ...
    function getLocalCityName(timezone) {
        const timezoneMap = {
            'America/New_York': 'New York', 'America/Chicago': 'Chicago', 'America/Denver': 'Denver', 'America/Los_Angeles': 'Los Angeles', 'Europe/London': 'Londra', 'Europe/Paris': 'Paris', 'Europe/Berlin': 'Berlin', 'Asia/Tokyo': 'Tokyo', 'Asia/Shanghai': 'Şanghay', 'Asia/Hong_Kong': 'Hong Kong', 'Asia/Singapore': 'Singapur', 'Australia/Sydney': 'Sidney', 'Australia/Melbourne': 'Melbourne', 'Pacific/Auckland': 'Auckland', 'America/Toronto': 'Toronto', 'America/Vancouver': 'Vancouver', 'Europe/Madrid': 'Madrid', 'Europe/Rome': 'Roma', 'Asia/Seoul': 'Seul', 'Asia/Mumbai': 'Mumbai', 'America/Sao_Paulo': 'São Paulo', 'America/Mexico_City': 'Meksiko', 'Africa/Cairo': 'Kahire', 'Africa/Johannesburg': 'Johannesburg'
        };
    ...
    function handleRowsMouseMove(percent, selectorLeft) {
        ...
        rows.forEach(row => {
            ...
            const timeFormatter = new Intl.DateTimeFormat('tr-TR', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
    ...
    function loadCityOrder() {
        try {
            ...
            console.warn('Şehir sırası yüklenemedi:', e);
        }
        return null;
    }
    ...
    function updateSingleRowTimeDisplay(row) {
        ...
        try {
            const timeFormatter = new Intl.DateTimeFormat('tr-TR', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
            const dateFormatter = new Intl.DateTimeFormat('tr-TR', { timeZone: timezone, weekday: 'short', month: 'short', day: 'numeric' });
            const timeString = timeFormatter.format(now);
            const dateString = dateFormatter.format(now);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Geçerli yerel saat';
            timeElement.innerHTML = `${timeString}<br><span style="font-size: 0.8em; color: #aaa;">${dateString}</span>`;
        } catch (error) {
            console.warn(`Saat dilimi ${timezone} için saat biçimlendirilemedi:`, error);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Geçersiz saat dilimi';
            timeElement.innerHTML = `Geçersiz<br><span style="font-size: 0.8em; color: #aaa;">Saat Dilimi</span>`;
        }
    }
    ...
    function renderTimelineGrid(row) {
        ...
        const hourFormatter = new Intl.DateTimeFormat('tr-TR', { timeZone: timezone, hour: 'numeric', hour12: false });
        const fullDateFormatter = new Intl.DateTimeFormat('tr-TR', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
        const weekdayFormatter = new Intl.DateTimeFormat('tr-TR', { timeZone: timezone, weekday: 'short' });
        const ymdPartsFormatter = new Intl.DateTimeFormat('tr-TR', { timeZone: timezone, year: 'numeric', month: '2-digit', day: '2-digit' });
        const now = new Date();
        ...
        for (let i = 0; i < TIMELINE_HOURS; i++) {
            ...
            if (weekdayShort === 'Cmt' || weekdayShort === 'Paz') {
            ...
            if (dateOfThisBlock === currentDateInTimezone && localHour === currentHourInTimezone) {
                ...
                currentTimeIndicator.textContent = 'ŞİMDİ';
            ...
            if (hourCount >= 2) {
                ...
                if (hourBlocks.length > 0) {
                    ...
                    if (startIndex >= 0) {
                        ...
                        const timezone = row.dataset.timezone;
                        const timezoneWeekdayFormatter = new Intl.DateTimeFormat('tr-TR', { timeZone: timezone, weekday: 'short' });
                        const timezoneDateFormatter = new Intl.DateTimeFormat('tr-TR', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
    ...
    function createMobileRangeOverlay(startPercent, endPercent) {
        ...
        button.textContent = 'Saatleri Görüntüle';
    ...
    function updateRangeInfo(timeRange) {
        ...
        document.getElementById('wtp-duration-display').textContent = `${durationHours}s ${durationMinutes}d`;
    }
    ...
    function updateTimezoneTimesDisplay(timezoneTimesData) {
        ...
        timezoneTimesData.forEach(data => {
            ...
            const startFormatted = start.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' });
            const endFormatted = end.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' });
            const startDateFormatted = start.toLocaleDateString('tr-TR', { weekday: 'short', month: 'short', day: 'numeric' });
    ...
    document.getElementById('wtp-google-meeting-btn').addEventListener('click', () => {
        ...
        const title = 'Planlanmış Toplantı';
        const details = 'Toplantı Detayları';
    ...
    document.getElementById('wtp-outlook-meeting-btn').addEventListener('click', () => {
        ...
        const { start, end } = currentTimeRange;
        const title = 'Planlanmış Toplantı';
        const url = `https://outlook.live.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(title)}&startdt=${start.toISOString()}&enddt=${end.toISOString()}&body=${encodeURIComponent('Toplantı Detayları')}`;
        window.open(url, '_blank');
    });
