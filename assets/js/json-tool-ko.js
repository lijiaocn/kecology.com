document.addEventListener('DOMContentLoaded', () => {
    ...
    const processJson = (isInitialLoad = false) => {
        ...
        try {
            ...
            updateStatus('유효한 JSON', false);
            renderOutputs();
            localStorage.setItem(STORAGE_KEY, jsonString);
        } catch (e) {
            currentJson = null;
            updateStatus(`잘못된 JSON: ${e.message}`, true);
            for (const key in outputs) {
                if (outputs[key]) {
                    if (key === 'tree') outputs[key].innerHTML = '';
                    else outputs[key].textContent = '';
    ...
    const renderOutputs = () => {
        ...
        try {
            ...
            outputs.base64.textContent = 'Base64로 인코딩하는 중 오류가 발생했습니다.';
        }
    ...
    function jsonToXml(json) {
        ...
        try {
            ...
            return `XML로 변환하는 중 오류가 발생했습니다: ${e.message}`;
        }
    }
    ...
    function jsonToYaml(json) {
        ...
        try {
            ...
            return `YAML로 변환하는 중 오류가 발생했습니다: ${e.message}`;
        }
    }
});
