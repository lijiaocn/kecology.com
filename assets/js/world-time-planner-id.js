document.addEventListener('DOMContentLoaded', () => {
    ...
    const countryTimeInfo = {
        'USA': { description: 'Amerika Serikat menggunakan 4 zona waktu: <strong>Waktu Bagian Timur (EST/EDT)</strong>, <strong>Waktu Bagian Tengah (CST/CDT)</strong>, <strong>Waktu Pegunungan (MST/MDT)</strong>, dan <strong>Waktu Pasifik (PST/PDT)</strong>. Menerapkan waktu musim panas.' },
        'Canada': { description: 'Kanada menggunakan 1 zona waktu: <strong>Waktu Bagian Timur (EST/EDT)</strong> dan menerapkan waktu musim panas.' },
        'Brazil': { description: 'Brasil menggunakan 1 zona waktu: <strong>Waktu Brasilia (BRT)</strong> dan tidak menerapkan waktu musim panas.' },
        'UK': { description: 'Britania Raya menggunakan 1 zona waktu: <strong>Waktu Greenwich (GMT)</strong> dan <strong>Waktu Musim Panas Britania (BST)</strong>, menerapkan waktu musim panas.' },
        'France': { description: 'Prancis menggunakan 1 zona waktu: <strong>Waktu Eropa Tengah (CET)</strong> dan <strong>Waktu Musim Panas Eropa Tengah (CEST)</strong>, menerapkan waktu musim panas.' },
        'Germany': { description: 'Jerman menggunakan 1 zona waktu: <strong>Waktu Eropa Tengah (CET)</strong> dan <strong>Waktu Musim Panas Eropa Tengah (CEST)</strong>, menerapkan waktu musim panas.' },
        'Russia': { description: 'Rusia menggunakan 1 zona waktu: <strong>Waktu Standar Moskow (MSK)</strong> dan tidak menerapkan waktu musim panas.' },
        'Egypt': { description: 'Mesir menggunakan 1 zona waktu: <strong>Waktu Eropa Timur (EET)</strong> dan <strong>Waktu Musim Panas Eropa Timur (EEST)</strong>, menerapkan waktu musim panas.' },
        'South Africa': { description: 'Afrika Selatan menggunakan 1 zona waktu: <strong>Waktu Standar Afrika Selatan (SAST)</strong> dan tidak menerapkan waktu musim panas.' },
        'UAE': { description: 'UEA menggunakan 1 zona waktu: <strong>Waktu Standar Teluk (GST)</strong> dan tidak menerapkan waktu musim panas.' },
        'India': { description: 'India menggunakan 1 zona waktu: <strong>Waktu Standar India (IST)</strong> dan tidak menerapkan waktu musim panas.' },
        'Singapore': { description: 'Singapura menggunakan 1 zona waktu: <strong>Waktu Standar Singapura (SGT)</strong> dan tidak menerapkan waktu musim panas.' },
        'China': { description: 'Cina menggunakan 1 zona waktu: <strong>Waktu Standar Cina (CST)</strong> dan tidak menerapkan waktu musim panas. Seluruh negara menggunakan waktu Beijing.' },
        'South Korea': { description: 'Korea Selatan menggunakan 1 zona waktu: <strong>Waktu Standar Korea (KST)</strong> dan tidak menerapkan waktu musim panas.' },
        'Japan': { description: 'Jepang menggunakan 1 zona waktu: <strong>Waktu Standar Jepang (JST)</strong> dan tidak menerapkan waktu musim panas.' },
        'Australia': { description: 'Australia menggunakan 1 zona waktu: <strong>Waktu Standar Timur Australia (AEST)</strong> dan <strong>Waktu Musim Panas Timur Australia (AEDT)</strong>, menerapkan waktu musim panas.' },
        'New Zealand': { description: 'Selandia Baru menggunakan 1 zona waktu: <strong>Waktu Standar Selandia Baru (NZST)</strong> dan <strong>Waktu Musim Panas Selandia Baru (NZDT)</strong>, menerapkan waktu musim panas.' }
    };
    ...
    function getLocalCityName(timezone) {
        const timezoneMap = {
            'America/New_York': 'New York', 'America/Chicago': 'Chicago', 'America/Denver': 'Denver', 'America/Los_Angeles': 'Los Angeles', 'Europe/London': 'London', 'Europe/Paris': 'Paris', 'Europe/Berlin': 'Berlin', 'Asia/Tokyo': 'Tokyo', 'Asia/Shanghai': 'Shanghai', 'Asia/Hong_Kong': 'Hong Kong', 'Asia/Singapore': 'Singapura', 'Australia/Sydney': 'Sydney', 'Australia/Melbourne': 'Melbourne', 'Pacific/Auckland': 'Auckland', 'America/Toronto': 'Toronto', 'America/Vancouver': 'Vancouver', 'Europe/Madrid': 'Madrid', 'Europe/Rome': 'Roma', 'Asia/Seoul': 'Seoul', 'Asia/Mumbai': 'Mumbai', 'America/Sao_Paulo': 'São Paulo', 'America/Mexico_City': 'Mexico City', 'Africa/Cairo': 'Kairo', 'Africa/Johannesburg': 'Johannesburg'
        };
    ...
    function handleRowsMouseMove(percent, selectorLeft) {
        ...
        rows.forEach(row => {
            ...
            const timeFormatter = new Intl.DateTimeFormat('id-ID', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
    ...
    function loadCityOrder() {
        try {
            ...
            console.warn('Gagal memuat urutan kota:', e);
        }
        return null;
    }
    ...
    function updateSingleRowTimeDisplay(row) {
        ...
        try {
            const timeFormatter = new Intl.DateTimeFormat('id-ID', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
            const dateFormatter = new Intl.DateTimeFormat('id-ID', { timeZone: timezone, weekday: 'short', month: 'short', day: 'numeric' });
            const timeString = timeFormatter.format(now);
            const dateString = dateFormatter.format(now);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Waktu lokal saat ini';
            timeElement.innerHTML = `${timeString}<br><span style="font-size: 0.8em; color: #aaa;">${dateString}</span>`;
        } catch (error) {
            console.warn(`Gagal memformat waktu untuk zona waktu ${timezone}:`, error);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Zona waktu tidak valid';
            timeElement.innerHTML = `Tidak Valid<br><span style="font-size: 0.8em; color: #aaa;">Zona Waktu</span>`;
        }
    }
    ...
    function renderTimelineGrid(row) {
        ...
        const hourFormatter = new Intl.DateTimeFormat('id-ID', { timeZone: timezone, hour: 'numeric', hour12: false });
        const fullDateFormatter = new Intl.DateTimeFormat('id-ID', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
        const weekdayFormatter = new Intl.DateTimeFormat('id-ID', { timeZone: timezone, weekday: 'short' });
        const ymdPartsFormatter = new Intl.DateTimeFormat('id-ID', { timeZone: timezone, year: 'numeric', month: '2-digit', day: '2-digit' });
        const now = new Date();
        ...
        for (let i = 0; i < TIMELINE_HOURS; i++) {
            ...
            if (weekdayShort === 'Sab' || weekdayShort === 'Min') {
            ...
            if (dateOfThisBlock === currentDateInTimezone && localHour === currentHourInTimezone) {
                ...
                currentTimeIndicator.textContent = 'SEKARANG';
            ...
            if (hourCount >= 2) {
                ...
                if (hourBlocks.length > 0) {
                    ...
                    if (startIndex >= 0) {
                        ...
                        const timezone = row.dataset.timezone;
                        const timezoneWeekdayFormatter = new Intl.DateTimeFormat('id-ID', { timeZone: timezone, weekday: 'short' });
                        const timezoneDateFormatter = new Intl.DateTimeFormat('id-ID', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
    ...
    function createMobileRangeOverlay(startPercent, endPercent) {
        ...
        button.textContent = 'Lihat Waktu';
    ...
    function updateRangeInfo(timeRange) {
        ...
        document.getElementById('wtp-duration-display').textContent = `${durationHours}j ${durationMinutes}m`;
    }
    ...
    function updateTimezoneTimesDisplay(timezoneTimesData) {
        ...
        timezoneTimesData.forEach(data => {
            ...
            const startDateFormatted = start.toLocaleDateString('id-ID', { weekday: 'short', month: 'short', day: 'numeric' });
    ...
    document.getElementById('wtp-google-meeting-btn').addEventListener('click', () => {
        ...
        const title = 'Rapat Terjadwal';
        const details = 'Detail Rapat';
    ...
    document.getElementById('wtp-outlook-meeting-btn').addEventListener('click', () => {
        ...
        const { start, end } = currentTimeRange;
        const title = 'Rapat Terjadwal';
        const url = `https://outlook.live.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(title)}&startdt=${start.toISOString()}&enddt=${end.toISOString()}&body=${encodeURIComponent('Detail Rapat')}`;
        window.open(url, '_blank');
    });
