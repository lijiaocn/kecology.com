document.addEventListener('DOMContentLoaded', () => {
    ...
    const processJson = (isInitialLoad = false) => {
        ...
        try {
            ...
            updateStatus('有効なJSON', false);
            renderOutputs();
            localStorage.setItem(STORAGE_KEY, jsonString);
        } catch (e) {
            currentJson = null;
            updateStatus(`無効なJSON: ${e.message}`, true);
            for (const key in outputs) {
                if (outputs[key]) {
                    if (key === 'tree') outputs[key].innerHTML = '';
                    else outputs[key].textContent = '';
    ...
    const renderOutputs = () => {
        ...
        try {
            ...
            outputs.base64.textContent = 'Base64へのエンコード中にエラーが発生しました。';
        }
    ...
    copyBtn.addEventListener('click', () => {
        if (currentJson === null) {
            updateStatus('コピーするものがありません。', true);
            return;
        }
...
`;
        ...
        try {
            ...
            return `XMLへの変換中にエラーが発生しました: ${e.message}`;
        }
    }
    ...
    function jsonToYaml(json) {
        ...
        try {
            ...
            return `YAMLへの変換中にエラーが発生しました: ${e.message}`;
        }
    }
});
