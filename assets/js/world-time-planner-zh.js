document.addEventListener('DOMContentLoaded', () => {
    ...
    const countryTimeInfo = {
        'USA': { description: '美国使用4个时区：<strong>东部时间(EST/EDT)</strong>、<strong>中部时间(CST/CDT)</strong>、<strong>山区时间(MST/MDT)</strong>和<strong>太平洋时间(PST/PDT)</strong>。实行夏令时。' },
        'Canada': { description: '加拿大使用1个时区：<strong>东部时间(EST/EDT)</strong>并实行夏令时。' },
        'Brazil': { description: '巴西使用1个时区：<strong>巴西利亚时间(BRT)</strong>，不实行夏令时。' },
        'UK': { description: '英国使用1个时区：<strong>格林尼治标准时间(GMT)</strong>和<strong>英国夏令时(BST)</strong>，实行夏令时。' },
        'France': { description: '法国使用1个时区：<strong>中欧时间(CET)</strong>和<strong>中欧夏令时(CEST)</strong>，实行夏令时。' },
        'Germany': { description: '德国使用1个时区：<strong>中欧时间(CET)</strong>和<strong>中欧夏令时(CEST)</strong>，实行夏令时。' },
        'Russia': { description: '俄罗斯使用1个时区：<strong>莫斯科标准时间(MSK)</strong>，不实行夏令时。' },
        'Egypt': { description: '埃及使用1个时区：<strong>东欧时间(EET)</strong>和<strong>东欧夏令时(EEST)</strong>，实行夏令时。' },
        'South Africa': { description: '南非使用1个时区：<strong>南非标准时间(SAST)</strong>，不实行夏令时。' },
        'UAE': { description: '阿联酋使用1个时区：<strong>海湾标准时间(GST)</strong>，不实行夏令时。' },
        'India': { description: '印度使用1个时区：<strong>印度标准时间(IST)</strong>，不实行夏令时。' },
        'Singapore': { description: '新加坡使用1个时区：<strong>新加坡标准时间(SGT)</strong>，不实行夏令时。' },
        'China': { description: '中国使用1个时区：<strong>中国标准时间(CST)</strong>，不实行夏令时。全国统一使用北京时间。' },
        'South Korea': { description: '韩国使用1个时区：<strong>韩国标准时间(KST)</strong>，不实行夏令时。' },
        'Japan': { description: '日本使用1个时区：<strong>日本标准时间(JST)</strong>，不实行夏令时。' },
        'Australia': { description: '澳大利亚使用1个时区：<strong>澳大利亚东部标准时间(AEST)</strong>和<strong>澳大利亚东部夏令时(AEDT)</strong>，实行夏令时。' },
        'New Zealand': { description: '新西兰使用1个时区：<strong>新西兰标准时间(NZST)</strong>和<strong>新西兰夏令时(NZDT)</strong>，实行夏令时。' }
    };
    ...
    function getLocalCityName(timezone) {
        const timezoneMap = {
            'America/New_York': '纽约', 'America/Chicago': '芝加哥', 'America/Denver': '丹佛', 'America/Los_Angeles': '洛杉矶', 'Europe/London': '伦敦', 'Europe/Paris': '巴黎', 'Europe/Berlin': '柏林', 'Asia/Tokyo': '东京', 'Asia/Shanghai': '上海', 'Asia/Hong_Kong': '香港', 'Asia/Singapore': '新加坡', 'Australia/Sydney': '悉尼', 'Australia/Melbourne': '墨尔本', 'Pacific/Auckland': '奥克兰', 'America/Toronto': '多伦多', 'America/Vancouver': '温哥华', 'Europe/Madrid': '马德里', 'Europe/Rome': '罗马', 'Asia/Seoul': '首尔', 'Asia/Mumbai': '孟买', 'America/Sao_Paulo': '圣保罗', 'America/Mexico_City': '墨西哥城', 'Africa/Cairo': '开罗', 'Africa/Johannesburg': '约翰内斯堡'
        };
    ...
    function handleRowsMouseMove(percent, selectorLeft) {
        ...
        rows.forEach(row => {
            ...
            const timeFormatter = new Intl.DateTimeFormat('zh-CN', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
    ...
    function updateSingleRowTimeDisplay(row) {
        ...
        try {
            const timeFormatter = new Intl.DateTimeFormat('zh-CN', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
            const dateFormatter = new Intl.DateTimeFormat('zh-CN', { timeZone: timezone, weekday: 'short', month: 'short', day: 'numeric' });
            const timeString = timeFormatter.format(now);
            const dateString = dateFormatter.format(now);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = '当前本地时间';
            timeElement.innerHTML = `${timeString}<br><span style="font-size: 0.8em; color: #aaa;">${dateString}</span>`;
        } catch (error) {
            console.warn(`无法格式化时区 ${timezone} 的时间:`, error);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = '无效时区';
            timeElement.innerHTML = `无效<br><span style="font-size: 0.8em; color: #aaa;">时区</span>`;
        }
    }
    ...
    function renderTimelineGrid(row) {
        ...
        const hourFormatter = new Intl.DateTimeFormat('zh-CN', { timeZone: timezone, hour: 'numeric', hour12: false });
        const fullDateFormatter = new Intl.DateTimeFormat('zh-CN', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
        const weekdayFormatter = new Intl.DateTimeFormat('zh-CN', { timeZone: timezone, weekday: 'short' });
        const ymdPartsFormatter = new Intl.DateTimeFormat('zh-CN', { timeZone: timezone, year: 'numeric', month: '2-digit', day: '2-digit' });
        const now = new Date();
        ...
        for (let i = 0; i < TIMELINE_HOURS; i++) {
            ...
            if (weekdayShort === '周六' || weekdayShort === '周日') {
            ...
            if (dateOfThisBlock === currentDateInTimezone && localHour === currentHourInTimezone) {
                ...
                currentTimeIndicator.textContent = '现在';
            ...
            if (hourCount >= 2) {
                ...
                if (hourBlocks.length > 0) {
                    ...
                    if (startIndex >= 0) {
                        ...
                        const timezone = row.dataset.timezone;
                        const timezoneWeekdayFormatter = new Intl.DateTimeFormat('zh-CN', { timeZone: timezone, weekday: 'short' });
                        const timezoneDateFormatter = new Intl.DateTimeFormat('zh-CN', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
    ...
    function calculateTimezoneTimes(timeRange) {
        ...
        timelineRows.forEach(row => {
            ...
            const cityName = cityElement ? cityElement.textContent : '未知';
            if (timezone) {
    ...
    function formatTimeInTimezone(date, timezone) {
        try {
            const formatter = new Intl.DateTimeFormat('zh-CN', { timeZone: timezone, year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false });
    ...
    function updateRangeInfo(timeRange) {
        ...
        document.getElementById('wtp-duration-display').textContent = `${durationHours}小时 ${durationMinutes}分钟`;
    }
    ...
    function updateTimezoneTimesDisplay(timezoneTimesData) {
        ...
        timezoneTimesData.forEach(data => {
            ...
            const startDateFormatted = start.toLocaleDateString('zh-CN', { weekday: 'short', month: 'short', day: 'numeric' });
    ...
    document.getElementById('wtp-google-meeting-btn').addEventListener('click', () => {
        ...
        const title = '预定会议';
        const details = '会议详情';
        const url = `https://www.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(title)}&dates=${start.toISOString().replace(/-|:|"\.\d+/g, '')}/${end.toISOString().replace(/-|:|"\.\d+/g, '')}&details=${encodeURIComponent(details)}`;
    ...
    document.getElementById('wtp-outlook-meeting-btn').addEventListener('click', () => {
        ...
        const { start, end } = currentTimeRange;
        const title = '预定会议';
        const url = `https://outlook.live.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(title)}&startdt=${start.toISOString()}&enddt=${end.toISOString()}&body=${encodeURIComponent('会议详情')}`;
        window.open(url, '_blank');
    });
    ...
    document.getElementById('wtp-yahoo-meeting-btn').addEventListener('click', () => {
        ...
        const url = `https://calendar.yahoo.com/?v=60&view=d&type=20&title=${encodeURIComponent(title)}&st=${start.toISOString().replace(/-|:|"\.\d+/g, '')}&dur=${durationHours}${durationMinutes}`;
        window.open(url, '_blank');
    });
