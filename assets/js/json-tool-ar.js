document.addEventListener('DOMContentLoaded', () => {
    ...
    const processJson = (isInitialLoad = false) => {
        ...
        try {
            ...
            updateStatus('JSON صالح', false);
            renderOutputs();
            localStorage.setItem(STORAGE_KEY, jsonString);
        } catch (e) {
            currentJson = null;
            updateStatus(`JSON غير صالح: ${e.message}`, true);
            for (const key in outputs) {
                if (outputs[key]) {
                    if (key === 'tree') outputs[key].innerHTML = '';
                    else outputs[key].textContent = '';
    ...
    const renderOutputs = () => {
        ...
        try {
            ...
            outputs.base64.textContent = 'خطأ في الترميز إلى Base64.';
        }
    ...
    copyBtn.addEventListener('click', () => {
        if (currentJson === null) {
            updateStatus('لا يوجد شيء لنسخه.', true);
            return;
        }
    ...
    function jsonToXml(json) {
        ...
        try {
            ...
            return `خطأ في التحويل إلى XML: ${e.message}`;
        }
    }
    ...
    function jsonToYaml(json) {
        ...
        try {
            ...
            return `خطأ في التحويل إلى YAML: ${e.message}`;
        }
    }
});
