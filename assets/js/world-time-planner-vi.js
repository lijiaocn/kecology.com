document.addEventListener('DOMContentLoaded', () => {
    ...
    const countryTimeInfo = {
        'USA': { description: 'Hoa Kỳ sử dụng 4 múi giờ: <strong>Giờ miền Đông (EST/EDT)</strong>, <strong>Giờ miền Trung (CST/CDT)</strong>, <strong>Giờ miền núi (MST/MDT)</strong>, và <strong>Giờ Thái Bình Dương (PST/PDT)</strong>. Có áp dụng giờ mùa hè.' },
        'Canada': { description: 'Canada sử dụng 1 múi giờ: <strong>Giờ miền Đông (EST/EDT)</strong> và có áp dụng giờ mùa hè.' },
        'Brazil': { description: 'Brazil sử dụng 1 múi giờ: <strong>Giờ Brasília (BRT)</strong> và không áp dụng giờ mùa hè.' },
        'UK': { description: 'Vương quốc Anh sử dụng 1 múi giờ: <strong>Giờ chuẩn Greenwich (GMT)</strong> và <strong>Giờ mùa hè của Anh (BST)</strong>, có áp dụng giờ mùa hè.' },
        'France': { description: 'Pháp sử dụng 1 múi giờ: <strong>Giờ Trung Âu (CET)</strong> và <strong>Giờ mùa hè Trung Âu (CEST)</strong>, có áp dụng giờ mùa hè.' },
        'Germany': { description: 'Đức sử dụng 1 múi giờ: <strong>Giờ Trung Âu (CET)</strong> và <strong>Giờ mùa hè Trung Âu (CEST)</strong>, có áp dụng giờ mùa hè.' },
        'Russia': { description: 'Nga sử dụng 1 múi giờ: <strong>Giờ chuẩn Moscow (MSK)</strong> và không áp dụng giờ mùa hè.' },
        'Egypt': { description: 'Ai Cập sử dụng 1 múi giờ: <strong>Giờ Đông Âu (EET)</strong> và <strong>Giờ mùa hè Đông Âu (EEST)</strong>, có áp dụng giờ mùa hè.' },
        'South Africa': { description: 'Nam Phi sử dụng 1 múi giờ: <strong>Giờ chuẩn Nam Phi (SAST)</strong> và không áp dụng giờ mùa hè.' },
        'UAE': { description: 'UAE sử dụng 1 múi giờ: <strong>Giờ chuẩn vùng Vịnh (GST)</strong> và không áp dụng giờ mùa hè.' },
        'India': { description: 'Ấn Độ sử dụng 1 múi giờ: <strong>Giờ chuẩn Ấn Độ (IST)</strong> và không áp dụng giờ mùa hè.' },
        'Singapore': { description: 'Singapore sử dụng 1 múi giờ: <strong>Giờ chuẩn Singapore (SGT)</strong> và không áp dụng giờ mùa hè.' },
        'China': { description: 'Trung Quốc sử dụng 1 múi giờ: <strong>Giờ chuẩn Trung Quốc (CST)</strong> và không áp dụng giờ mùa hè. Toàn bộ đất nước sử dụng giờ Bắc Kinh.' },
        'South Korea': { description: 'Hàn Quốc sử dụng 1 múi giờ: <strong>Giờ chuẩn Hàn Quốc (KST)</strong> và không áp dụng giờ mùa hè.' },
        'Japan': { description: 'Nhật Bản sử dụng 1 múi giờ: <strong>Giờ chuẩn Nhật Bản (JST)</strong> và không áp dụng giờ mùa hè.' },
        'Australia': { description: 'Úc sử dụng 1 múi giờ: <strong>Giờ chuẩn miền Đông Úc (AEST)</strong> và <strong>Giờ mùa hè miền Đông Úc (AEDT)</strong>, có áp dụng giờ mùa hè.' },
        'New Zealand': { description: 'New Zealand sử dụng 1 múi giờ: <strong>Giờ chuẩn New Zealand (NZST)</strong> và <strong>Giờ mùa hè New Zealand (NZDT)</strong>, có áp dụng giờ mùa hè.' }
    };
    ...
    function getLocalCityName(timezone) {
        const timezoneMap = {
            'America/New_York': 'New York', 'America/Chicago': 'Chicago', 'America/Denver': 'Denver', 'America/Los_Angeles': 'Los Angeles', 'Europe/London': 'Luân Đôn', 'Europe/Paris': 'Paris', 'Europe/Berlin': 'Berlin', 'Asia/Tokyo': 'Tokyo', 'Asia/Shanghai': 'Thượng Hải', 'Asia/Hong_Kong': 'Hồng Kông', 'Asia/Singapore': 'Singapore', 'Australia/Sydney': 'Sydney', 'Australia/Melbourne': 'Melbourne', 'Pacific/Auckland': 'Auckland', 'America/Toronto': 'Toronto', 'America/Vancouver': 'Vancouver', 'Europe/Madrid': 'Madrid', 'Europe/Rome': 'Rome', 'Asia/Seoul': 'Seoul', 'Asia/Mumbai': 'Mumbai', 'America/Sao_Paulo': 'São Paulo', 'America/Mexico_City': 'Thành phố Mexico', 'Africa/Cairo': 'Cairo', 'Africa/Johannesburg': 'Johannesburg'
        };
    ...
    function handleRowsMouseMove(percent, selectorLeft) {
        ...
        rows.forEach(row => {
            ...
            const timeFormatter = new Intl.DateTimeFormat('vi-VN', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
    ...
    function loadCityOrder() {
        try {
            ...
            console.warn('Không thể tải thứ tự thành phố:', e);
        }
        return null;
    }
    ...
    function updateSingleRowTimeDisplay(row) {
        ...
        try {
            const timeFormatter = new Intl.DateTimeFormat('vi-VN', { timeZone: timezone, hour: 'numeric', minute: 'numeric', hour12: false });
            const dateFormatter = new Intl.DateTimeFormat('vi-VN', { timeZone: timezone, weekday: 'short', month: 'short', day: 'numeric' });
            const timeString = timeFormatter.format(now);
            const dateString = dateFormatter.format(now);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Giờ địa phương hiện tại';
            timeElement.innerHTML = `${timeString}<br><span style="font-size: 0.8em; color: #aaa;">${dateString}</span>`;
        } catch (error) {
            console.warn(`Không thể định dạng thời gian cho múi giờ ${timezone}:`, error);
            const timeElement = row.querySelector('.wtp-current-time');
            timeElement.title = 'Múi giờ không hợp lệ';
            timeElement.innerHTML = `Không hợp lệ<br><span style="font-size: 0.8em; color: #aaa;">Múi giờ</span>`;
        }
    }
    ...
    function renderTimelineGrid(row) {
        ...
        const hourFormatter = new Intl.DateTimeFormat('vi-VN', { timeZone: timezone, hour: 'numeric', hour12: false });
        const fullDateFormatter = new Intl.DateTimeFormat('vi-VN', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
        const weekdayFormatter = new Intl.DateTimeFormat('vi-VN', { timeZone: timezone, weekday: 'short' });
        const ymdPartsFormatter = new Intl.DateTimeFormat('vi-VN', { timeZone: timezone, year: 'numeric', month: '2-digit', day: '2-digit' });
        const now = new Date();
        ...
        for (let i = 0; i < TIMELINE_HOURS; i++) {
            ...
            if (weekdayShort === 'Th 7' || weekdayShort === 'CN') {
            ...
            if (dateOfThisBlock === currentDateInTimezone && localHour === currentHourInTimezone) {
                ...
                currentTimeIndicator.textContent = 'BÂY GIỜ';
            ...
            if (hourCount >= 2) {
                ...
                if (hourBlocks.length > 0) {
                    ...
                    if (startIndex >= 0) {
                        ...
                        const timezone = row.dataset.timezone;
                        const timezoneWeekdayFormatter = new Intl.DateTimeFormat('vi-VN', { timeZone: timezone, weekday: 'short' });
                        const timezoneDateFormatter = new Intl.DateTimeFormat('vi-VN', { timeZone: timezone, year: 'numeric', month: 'numeric', day: 'numeric' });
    ...
    function updateTimezoneTimesDisplay(timezoneTimesData) {
        ...
        timezoneTimesData.forEach(data => {
            ...
            const startDateFormatted = start.toLocaleDateString('vi-VN', { weekday: 'short', month: 'short', day: 'numeric' });
    ...
    document.getElementById('wtp-google-meeting-btn').addEventListener('click', () => {
        ...
        const title = 'Cuộc họp đã lên lịch';
        const details = 'Chi tiết cuộc họp';
    ...
    document.getElementById('wtp-outlook-meeting-btn').addEventListener('click', () => {
        ...
        const { start, end } = currentTimeRange;
        const title = 'Cuộc họp đã lên lịch';
        const url = `https://outlook.live.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(title)}&startdt=${start.toISOString()}&enddt=${end.toISOString()}&body=${encodeURIComponent('Chi tiết cuộc họp')}`;
        window.open(url, '_blank');
    });
