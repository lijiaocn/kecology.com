---
layout: page
title: "就寝時間計算機：すっきり目覚めるには何時に寝るべき？"
date:   2025-08-15 17:25:46 +0800
last_modified_at: 2025-09-09 14:59:00 +0800
categories: tool
lang: ja
permalink: /ja/tool/best-bedtime-calculator
description: "90分の睡眠サイクルに基づいて、最適な就寝時間を見つけます。すっきりとエネルギッシュに目覚めるのに役立ちます。"
mysetting:
  nowrapper: false
---
...
<div id="sleep-calculator-container">
    <h1>就寝時間計算機</h1>
    <p>起きる時間を設定して、いつ寝るべきかを確認しましょう。</p>
    <div id="controls">
    ...
    <p class="keyboard-hint">ショートカット： <kbd>↑</kbd> <kbd>↓</kbd>で調整、<kbd>←</kbd> <kbd>→</kbd>で切り替え</p>
    <div id="results-container" style="display: none;">
        <h3 id="results-title"><span id="wake-up-time"></span>にすっきりと目覚めるには、次のいずれかの時間に眠りにつくようにしてください： <small style="font-weight: normal;">(入眠に15分、睡眠サイクル90分を含む)</small></h3>
        ...
        <div style="background-color: #1c2b3a; border-left: 5px solid #00c7b4; padding: 15px; margin: 20px 0 0 0; border-radius: 4px; color: #e0e0e0; font-size: 15px; text-align: left;">
          <p style="margin: 0 0 10px 0; font-weight: bold;"><span style="color: #ff6b6b;">❤</span> 英国心臓財団からのアドバイス： <a href="https://www.bhf.org.uk/informationsupport/heart-matters-magazine/news/behind-the-headlines/sleep-time-and-heart-disease" target="_blank" style="color: #8492a6; text-decoration: none; font-size: 12px;">(続きを読む)</a></p>
          <ul style="margin: 0; padding-left: 0; list-style-type: none;">
            <li style="margin-bottom: 10px; padding-left: 20px; position: relative;">
              <span style="position: absolute; left: 0; color: #00c7b4;">✔</span>
              より健康な心臓のために<strong>午後10時から11時</strong>の間に寝ましょう。
            </li>
            <li style="padding-left: 20px; position: relative;">
              <span style="position: absolute; left: 0; color: #00c7b4;">✔</span>
              ほとんどの成人は、一晩に<strong>7〜9時間</strong>の睡眠を目指すべきです。
            </li>
          </ul>
        </div>
    </div>
    ...
    <div id="suggestion-now">
        <h3>今すぐ寝ますか？</h3>
        <a href="/ja/tool/wakeup-calculator" class="bedtime-option-btn" style="text-decoration: none;">起床時間計算機をお試しください</a>
    </div>
</div>
...
<script>
    ...
    function createScroller(containerId, items, prevScrollerId, nextScrollerId) {
        ...
        container.addEventListener('wheel', e => {
            ...
            realTranslateY -= e.deltaY * 0.5; // 感度調整
        ...
        // キーボードとクリックのサポート
        container.tabIndex = 0; // フォーカス可能にする
        container.style.outline = 'none'; // デフォルトのアウトラインを削除

        container.addEventListener('focus', () => {
            selection.style.borderColor = '#ffc107'; // フォーカス時にハイライト
        });
        container.addEventListener('blur', () => {
            selection.style.borderColor = '#00c7b4'; // ブラー時に元に戻す
        });
    ...
    function calculateBedtimes() {
        ...
        if (ampmVal === '午後' && hour < 12) {
            hour += 12;
        }
        if (ampmVal === '午前' && hour === 12) {
            hour = 0;
        }
        ...
        bedtimes.forEach(bt => {
            ...
            durationSpan.textContent = `(${bt.duration}の睡眠)`;
    ...
    function setInitialTime() {
        ...
        const initialAmpm = initialHour >= 12 ? '午後' : '午前';
