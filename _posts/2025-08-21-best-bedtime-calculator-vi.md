---
layout: page
title: "Máy tính giờ đi ngủ: Tôi cần đi ngủ lúc mấy giờ để cảm thấy sảng khoái?"
date:   2025-08-15 17:25:46 +0800
last_modified_at: 2025-09-09 14:59:00 +0800
categories: tool
lang: vi
permalink: /vi/tool/best-bedtime-calculator
description: "Tìm thời gian tốt nhất để đi ngủ dựa trên chu kỳ giấc ngủ 90 phút. Nó có thể giúp bạn thức dậy với cảm giác sảng khoái và tràn đầy năng lượng."
mysetting:
  nowrapper: false
---
...
<div id="sleep-calculator-container">
    <h1>Máy tính giờ đi ngủ</h1>
    <p>Đặt giờ thức dậy của bạn để xem bạn nên đi ngủ khi nào.</p>
    <div id="controls">
    ...
    <p class="keyboard-hint">Phím tắt: <kbd>↑</kbd> <kbd>↓</kbd> để điều chỉnh và <kbd>←</kbd> <kbd>→</kbd> để chuyển đổi</p>
    <div id="results-container" style="display: none;">
        <h3 id="results-title">Để thức dậy sảng khoái lúc <span id="wake-up-time"></span>, hãy thử đi vào giấc ngủ vào một trong những thời điểm sau: <small style="font-weight: normal;">(Bao gồm 15 phút để chìm vào giấc ngủ & chu kỳ ngủ 90 phút)</small></h3>
        ...
        <div style="background-color: #1c2b3a; border-left: 5px solid #00c7b4; padding: 15px; margin: 20px 0 0 0; border-radius: 4px; color: #e0e0e0; font-size: 15px; text-align: left;">
          <p style="margin: 0 0 10px 0; font-weight: bold;"><span style="color: #ff6b6b;">❤</span> Lời khuyên của Quỹ Tim mạch Anh: <a href="https://www.bhf.org.uk/informationsupport/heart-matters-magazine/news/behind-the-headlines/sleep-time-and-heart-disease" target="_blank" style="color: #8492a6; text-decoration: none; font-size: 12px;">(đọc thêm)</a></p>
          <ul style="margin: 0; padding-left: 0; list-style-type: none;">
            <li style="margin-bottom: 10px; padding-left: 20px; position: relative;">
              <span style="position: absolute; left: 0; color: #00c7b4;">✔</span>
              Ngủ từ <strong>10 giờ tối đến 11 giờ tối</strong> để có một trái tim khỏe mạnh hơn.
            </li>
            <li style="padding-left: 20px; position: relative;">
              <span style="position: absolute; left: 0; color: #00c7b4;">✔</span>
              Hầu hết người lớn nên ngủ từ <strong>7 đến 9 giờ</strong> mỗi đêm.
            </li>
          </ul>
        </div>
    </div>
    ...
    <div id="suggestion-now">
        <h3>Đi ngủ ngay bây giờ?</h3>
        <a href="/vi/tool/wakeup-calculator" class="bedtime-option-btn" style="text-decoration: none;">Hãy thử Máy tính giờ thức dậy của chúng tôi</a>
    </div>
</div>
...
<script>
    ...
    function createScroller(containerId, items, prevScrollerId, nextScrollerId) {
        ...
        container.addEventListener('wheel', e => {
            ...
            realTranslateY -= e.deltaY * 0.5; // Điều chỉnh độ nhạy
        ...
        // Hỗ trợ bàn phím và nhấp chuột
        container.tabIndex = 0; // Cho phép lấy tiêu điểm
        container.style.outline = 'none'; // Xóa đường viền mặc định

        container.addEventListener('focus', () => {
            selection.style.borderColor = '#ffc107'; // Đánh dấu khi có tiêu điểm
        });
        container.addEventListener('blur', () => {
            selection.style.borderColor = '#00c7b4'; // Hoàn nguyên khi mất tiêu điểm
        });
    ...
    function calculateBedtimes() {
        ...
        if (ampmVal === 'CH' && hour < 12) {
            hour += 12;
        }
        if (ampmVal === 'SA' && hour === 12) {
            hour = 0;
        }
        ...
        bedtimes.forEach(bt => {
            ...
            durationSpan.textContent = `(${bt.duration} ngủ)`;
    ...
    function setInitialTime() {
        ...
        const initialAmpm = initialHour >= 12 ? 'CH' : 'SA';
